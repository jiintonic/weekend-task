    ...
    switch (*start_position)
      {
        case 0:
          {
            set_diagnostic_source_file_name(
                    the_tokenizer->next_token_location.file_name);
            set_diagnostic_source_line_number(line_number);
            set_diagnostic_source_column_number(column_number);
            set_diagnostic_source_character_count(0);
            basic_error("Unterminated %s literal.", literal_description);
            unset_diagnostic_source_character_count();
                    ...
                    if (value == 0)
                      {
                        ...
                        set_diagnostic_source_line_number(line_number);
                        set_diagnostic_source_column_number(column_number);
                        set_diagnostic_source_character_count(
                                *input_characters_used);
                        basic_error(
                                "In %s literal, an octal escape sequence "
                    ...
                    if (!(is_hex_digit(next_character)))
                      {
                        ...
                        set_diagnostic_source_line_number(line_number);
                        set_diagnostic_source_column_number(column_number);
                        set_diagnostic_source_character_count(2);
                        basic_error(
                                "In %s literal, a hexadecimal escape sequence "
                                "had zero hex digits.", literal_description);
                        unset_diagnostic_source_character_count();
                        unset_diagnostic_source_column_number();
                        unset_diagnostic_source_line_number();
                        unset_diagnostic_source_file_name();
                        ...
                        if (value > 0x10ffff)
                          {
                            ...
                            basic_error(
                                    "In %s literal, a hexadecimal escape "
                                    "sequence gives a value greater than "
                                    "0x10ffff, the maximum Unicode code "
                                    "point.", literal_description);
                            unset_diagnostic_source_character_count();
                            unset_diagnostic_source_column_number();
                            unset_diagnostic_source_line_number();
                            unset_diagnostic_source_file_name();
                ...
                case 0:
                  {
                    set_diagnostic_source_file_name(
                            the_tokenizer->next_token_location.file_name);
                    set_diagnostic_source_line_number(line_number);
                    set_diagnostic_source_column_number(column_number + 1);
                    set_diagnostic_source_character_count(0);
                    basic_error("Unterminated %s literal.",
                                literal_description);
                    unset_diagnostic_source_character_count();
                    unset_diagnostic_source_column_number();
                    unset_diagnostic_source_line_number();
        ...
        default:
          {
            ...
            set_diagnostic_source_line_number(line_number);
            set_diagnostic_source_column_number(column_number);
            set_diagnostic_source_character_count(1);
                ...
                if (next_character != '\'')
                  {
                    set_diagnostic_source_file_name(
                            the_tokenizer->next_token_location.file_name);
                    set_diagnostic_source_line_number(
                            the_tokenizer->next_token_location.
                                    start_line_number);
            ...
            case '9':
              {
                ...
                set_diagnostic_source_character_count(position - start);
                    ...
                    (*position == 'E'))
                  {
                    ...
                    if ((*position == '.') && (position[1] >= '0') &&
                        (position[1] <= '9'))
                      {
                        ...
                        column_number += (position - decimal_start);

                        assert(position > decimal_start);

                        set_diagnostic_source_character_count(
                                position - start);

                        oi_create_from_decimal_ascii(numerator,
                                (position - decimal_start), decimal_start,
                                FALSE);
                        ...
                        oi_create_from_size_t(exponent,
                                              position - decimal_start);
                    ...
                    if ((*position == 'e') || (*position == 'E'))
                      {
                        ...
                        start_exponent = position;
                        do
                          {
                            ++position;
                          } while ((*position >= '0') && (*position <= '9'));

                        column_number += (position - start_exponent);

                        set_diagnostic_source_character_count(
                                position - start);

                        oi_create_from_decimal_ascii(exponent,
                                (position - start_exponent), start_exponent,
                                FALSE);
                        if (oi_out_of_memory(exponent))
                          {
                            unset_diagnostic_source_character_count();
            ...
            case '?':
              {
                if (position[1] == '?')
                  {
                    const char *start_marker_end;
                    const char *text_start;
                    const char *match;
                    const char *end_marker_end;
                    const char *text_end;

                    start_marker_end = position + 2;
                    while (*start_marker_end != '\n')
                      {
                        if ((*start_marker_end == 0) ||
                            (*start_marker_end == '?'))
                          {
                            set_diagnostic_source_file_name(
                                    the_tokenizer->next_token_location.
                            ...
                            set_diagnostic_source_column_number(column_number);
                            set_diagnostic_source_character_count(
                                    start_marker_end - position);
                            basic_error(
                                    (*start_marker_end == 0) ?
                                    "Unterminated block string literal." :
                                    "Question mark in block string literal "
                                    "marker.");
                            unset_diagnostic_source_character_count();
                            unset_diagnostic_source_column_number();
                            unset_diagnostic_source_line_number();
                            unset_diagnostic_source_file_name();
                            result = create_error_token();
                            column_number += (start_marker_end - position);
                            position = start_marker_end;
                            goto ready_to_break;
                          }
                        ++start_marker_end;
                      }

                    text_start = start_marker_end + 1;
                    match = position;
                    end_marker_end = text_start;
                    while (TRUE)
                      {
                        if (*end_marker_end == 0)
                          {
                            ...
                            set_diagnostic_source_column_number(column_number);
                            set_diagnostic_source_character_count(
                                    end_marker_end - position);
                            basic_error("Unterminated block string literal.");
                            unset_diagnostic_source_character_count();
                            unset_diagnostic_source_column_number();
                            unset_diagnostic_source_line_number();
                            unset_diagnostic_source_file_name();
                            ...
                            position = end_marker_end;
                            goto ready_to_break;
                          }
                        if (*end_marker_end == *match)
                          {
                            ++match;
                            if (match == start_marker_end)
                              {
                                ++end_marker_end;
                                break;
                              }
                    ...
                    text_end = end_marker_end - (start_marker_end - position);
                    if (text_end == text_start)
                      {
                        result = create_string_literal_token("");
                      }
                    else
                      {
                        char *copy;

                        copy = MALLOC_ARRAY(char, (text_end - text_start) + 1);
                        if (copy == NULL)
                          {
                            the_tokenizer->error = TRUE;
                            return NULL;
                          }
                        memcpy(copy, text_start, (text_end - text_start));
                        copy[text_end - text_start] = 0;
                        result = create_string_literal_token(copy);
                        free(copy);
                      }
                ...
                while (*follow != '@')
                  {
                    ...
                    if (*follow == 0)
                      {
                        set_diagnostic_source_character_count(character_count);
                        basic_error(
                    ...
                    set_diagnostic_source_character_count(character_count + 1);
                ...
                if (the_regular_expression == NULL)
                  {
                    ...
                    set_diagnostic_source_column_number(
                            the_tokenizer->next_token_location.
                                    start_column_number);
                    set_diagnostic_source_character_count(length + 2);
                    basic_error("%s",
