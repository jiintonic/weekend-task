/* file "RadioToAMCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef RADIOTOAMCOMMANDJSON_H
#define RADIOTOAMCOMMANDJSON_H
...
class RadioToAMCommandJSON : public RadioCommandJSON
  {
  private:
    ...
    RadioToAMCommandJSON(const RadioToAMCommandJSON &);
    RadioToAMCommandJSON & operator=(const RadioToAMCommandJSON &other);
  ...
  public:
    RadioToAMCommandJSON(void);
    virtual ~RadioToAMCommandJSON(void);
    ...
    virtual size_t extraRadioToAMCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraRadioToAMCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraRadioToAMCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraRadioToAMCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraRadioToAMCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraRadioToAMCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraRadioToAMCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraRadioToAMCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static RadioToAMCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static RadioToAMCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        RadioToAMCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for RadioToAMCommandJSON", &generator);
    ...
    static RadioToAMCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        RadioToAMCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<RadioToAMCommandJSON>, RadioToAMCommandJSON *, bool> generator("Type RadioToAMCommand", ignore_extras);
    ...
    class Generator : public RadioCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getRadioCommandJSONKey().c_str(), "RadioToAM") == 0))
                throw("The key field has a value other than `RadioToAM'.");
            RadioToAMCommandJSON *result = new RadioToAMCommandJSON();
            assert(result != NULL);
            RCHandle<RadioToAMCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraRadioToAMCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(RadioToAMCommandJSON *result)
          {
        ...
        virtual void handle_result(RadioToAMCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : RadioCommandJSON::Generator(ignore_extras), fieldGeneratorAlreadyOnResult("field \"AlreadyOnResult\" of the RadioToAMCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
