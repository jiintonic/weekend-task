/* file "CameraCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef CAMERACOMMANDJSON_H
#define CAMERACOMMANDJSON_H
...
class CameraCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    CameraCommandJSON(const CameraCommandJSON &);
    CameraCommandJSON & operator=(const CameraCommandJSON &other);
  ...
  public:
    CameraCommandJSON(void);
    virtual ~CameraCommandJSON(void);
    ...
    virtual size_t extraCameraCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraCameraCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraCameraCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraCameraCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraCameraCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraCameraCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraCameraCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraCameraCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static CameraCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static CameraCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        CameraCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for CameraCommandJSON", &generator);
    ...
    static CameraCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        CameraCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<CameraCommandJSON>, CameraCommandJSON *, bool> generator("Type CameraCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "CameraCommand") == 0))
                throw("The key field has a value other than `CameraCommand'.");
            CameraCommandJSON *result = new CameraCommandJSON();
            assert(result != NULL);
            RCHandle<CameraCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraCameraCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(CameraCommandJSON *result)
          {
            if (fieldGeneratorCommandType.have_value)
              {
                result->setCommandType();
        ...
        virtual void handle_result(CameraCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the CameraCommand class"), unknownFieldGenerator(ignore_extras)
          {
