/* file "SetClientUberServerTokenCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef SETCLIENTUBERSERVERTOKENCOMMANDJSON_H
#define SETCLIENTUBERSERVERTOKENCOMMANDJSON_H
...
class SetClientUberServerTokenCommandJSON : public UberCommandJSON
  {
  public:
  ...
  private:
    ...
    SetClientUberServerTokenCommandJSON(const SetClientUberServerTokenCommandJSON &);
    SetClientUberServerTokenCommandJSON & operator=(const SetClientUberServerTokenCommandJSON &other);
  ...
  public:
    SetClientUberServerTokenCommandJSON(void);
    virtual ~SetClientUberServerTokenCommandJSON(void);
    ...
    virtual size_t extraSetClientUberServerTokenCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraSetClientUberServerTokenCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraSetClientUberServerTokenCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraSetClientUberServerTokenCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraSetClientUberServerTokenCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraSetClientUberServerTokenCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraSetClientUberServerTokenCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraSetClientUberServerTokenCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static SetClientUberServerTokenCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static SetClientUberServerTokenCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        SetClientUberServerTokenCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for SetClientUberServerTokenCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static SetClientUberServerTokenCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        SetClientUberServerTokenCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<SetClientUberServerTokenCommandJSON>, SetClientUberServerTokenCommandJSON *, bool> generator("Type SetClientUberServerTokenCommand", ignore_extras);
    ...
    class Generator : public UberCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberCommandJSONKey().c_str(), "SetClientUberServerToken") == 0))
                throw("The key field has a value other than `SetClientUberServerToken'.");
            SetClientUberServerTokenCommandJSON *result = new SetClientUberServerTokenCommandJSON();
            assert(result != NULL);
            RCHandle<SetClientUberServerTokenCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraSetClientUberServerTokenCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(SetClientUberServerTokenCommandJSON *result)
          {
        ...
        virtual void handle_result(SetClientUberServerTokenCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the SetClientUberServerTokenCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* SETCLIENTUBERSERVERTOKENCOMMANDJSON_H */
