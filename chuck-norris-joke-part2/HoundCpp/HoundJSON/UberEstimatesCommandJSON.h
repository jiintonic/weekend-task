/* file "UberEstimatesCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UBERESTIMATESCOMMANDJSON_H
#define UBERESTIMATESCOMMANDJSON_H
...
class UberEstimatesCommandJSON : public UberCommandJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      public:
        ...
        class Generator : public JSONObjectGenerator
          ...
          public:
            Generator(bool ignore_extras = false) : fieldGeneratorUberEstimatesSpec("field \"UberEstimatesSpec\" of the TypeNativeData class", ignore_extras), fieldGeneratorUberProducts("field \"UberProducts\" of the TypeNativeData class", ignore_extras), unknownFieldGenerator(ignore_extras)
              {
  ...
  private:
    ...
    UberEstimatesCommandJSON(const UberEstimatesCommandJSON &);
    UberEstimatesCommandJSON & operator=(const UberEstimatesCommandJSON &other);
  ...
  public:
    UberEstimatesCommandJSON(void);
    virtual ~UberEstimatesCommandJSON(void);
    ...
    virtual size_t extraUberEstimatesCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUberEstimatesCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUberEstimatesCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUberEstimatesCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUberEstimatesCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraUberEstimatesCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraUberEstimatesCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraUberEstimatesCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UberEstimatesCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UberEstimatesCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UberEstimatesCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for UberEstimatesCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static UberEstimatesCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UberEstimatesCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UberEstimatesCommandJSON>, UberEstimatesCommandJSON *, bool> generator("Type UberEstimatesCommand", ignore_extras);
    ...
    class Generator : public UberCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberCommandJSONKey().c_str(), "UberEstimates") == 0))
                throw("The key field has a value other than `UberEstimates'.");
            UberEstimatesCommandJSON *result = new UberEstimatesCommandJSON();
            assert(result != NULL);
            RCHandle<UberEstimatesCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUberEstimatesCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(UberEstimatesCommandJSON *result)
          {
        ...
        virtual void handle_result(UberEstimatesCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the UberEstimatesCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* UBERESTIMATESCOMMANDJSON_H */
