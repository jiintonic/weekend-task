/* file "PowerCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef POWERCOMMANDJSON_H
#define POWERCOMMANDJSON_H
...
class PowerCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    PowerCommandJSON(const PowerCommandJSON &);
    PowerCommandJSON & operator=(const PowerCommandJSON &other);
  ...
  public:
    PowerCommandJSON(void);
    virtual ~PowerCommandJSON(void);
    ...
    virtual size_t extraPowerCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraPowerCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraPowerCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraPowerCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraPowerCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraPowerCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraPowerCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraPowerCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static PowerCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static PowerCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        PowerCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for PowerCommandJSON", &generator);
    ...
    static PowerCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        PowerCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<PowerCommandJSON>, PowerCommandJSON *, bool> generator("Type PowerCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "PowerCommand") == 0))
                throw("The key field has a value other than `PowerCommand'.");
            PowerCommandJSON *result = new PowerCommandJSON();
            assert(result != NULL);
            RCHandle<PowerCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraPowerCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(PowerCommandJSON *result)
          {
        ...
        virtual void handle_result(PowerCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the PowerCommand class"), unknownFieldGenerator(ignore_extras)
          {
