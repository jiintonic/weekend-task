/* file "WikipediaInformationNuggetJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef WIKIPEDIAINFORMATIONNUGGETJSON_H
#define WIKIPEDIAINFORMATIONNUGGETJSON_H
...
class WikipediaInformationNuggetJSON : public InformationNuggetJSON
  {
  public:
  ...
  private:
    ...
    WikipediaInformationNuggetJSON(const WikipediaInformationNuggetJSON &);
    WikipediaInformationNuggetJSON & operator=(const WikipediaInformationNuggetJSON &other);
  ...
  public:
    WikipediaInformationNuggetJSON(void);
    virtual ~WikipediaInformationNuggetJSON(void);
    ...
    virtual size_t extraWikipediaInformationNuggetComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraWikipediaInformationNuggetComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraWikipediaInformationNuggetComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraWikipediaInformationNuggetComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraWikipediaInformationNuggetLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraWikipediaInformationNuggetLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraWikipediaInformationNuggetAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraWikipediaInformationNuggetSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static WikipediaInformationNuggetJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static WikipediaInformationNuggetJSON *from_text(const char *text, bool ignore_extras = false)
      {
        WikipediaInformationNuggetJSON *result;
          {
            ...
            parse_json_value(text, "Text for WikipediaInformationNuggetJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static WikipediaInformationNuggetJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        WikipediaInformationNuggetJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<WikipediaInformationNuggetJSON>, WikipediaInformationNuggetJSON *, bool> generator("Type WikipediaInformationNugget", ignore_extras);
    ...
    class Generator : public InformationNuggetJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getInformationNuggetJSONKey().c_str(), "Wikipedia") == 0))
                throw("The key field has a value other than `Wikipedia'.");
            WikipediaInformationNuggetJSON *result = new WikipediaInformationNuggetJSON();
            assert(result != NULL);
            RCHandle<WikipediaInformationNuggetJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraWikipediaInformationNuggetAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(WikipediaInformationNuggetJSON *result)
          {
        ...
        virtual void handle_result(WikipediaInformationNuggetJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : InformationNuggetJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the WikipediaInformationNugget class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* WIKIPEDIAINFORMATIONNUGGETJSON_H */
