/* file "SetClientUberClientIdCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef SETCLIENTUBERCLIENTIDCOMMANDJSON_H
#define SETCLIENTUBERCLIENTIDCOMMANDJSON_H
...
class SetClientUberClientIdCommandJSON : public UberCommandJSON
  {
  public:
  ...
  private:
    ...
    SetClientUberClientIdCommandJSON(const SetClientUberClientIdCommandJSON &);
    SetClientUberClientIdCommandJSON & operator=(const SetClientUberClientIdCommandJSON &other);
  ...
  public:
    SetClientUberClientIdCommandJSON(void);
    virtual ~SetClientUberClientIdCommandJSON(void);
    ...
    virtual size_t extraSetClientUberClientIdCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraSetClientUberClientIdCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraSetClientUberClientIdCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraSetClientUberClientIdCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraSetClientUberClientIdCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraSetClientUberClientIdCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraSetClientUberClientIdCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraSetClientUberClientIdCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static SetClientUberClientIdCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static SetClientUberClientIdCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        SetClientUberClientIdCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for SetClientUberClientIdCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static SetClientUberClientIdCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        SetClientUberClientIdCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<SetClientUberClientIdCommandJSON>, SetClientUberClientIdCommandJSON *, bool> generator("Type SetClientUberClientIdCommand", ignore_extras);
    ...
    class Generator : public UberCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberCommandJSONKey().c_str(), "SetClientUberClientId") == 0))
                throw("The key field has a value other than `SetClientUberClientId'.");
            SetClientUberClientIdCommandJSON *result = new SetClientUberClientIdCommandJSON();
            assert(result != NULL);
            RCHandle<SetClientUberClientIdCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraSetClientUberClientIdCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(SetClientUberClientIdCommandJSON *result)
          {
        ...
        virtual void handle_result(SetClientUberClientIdCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the SetClientUberClientIdCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* SETCLIENTUBERCLIENTIDCOMMANDJSON_H */
