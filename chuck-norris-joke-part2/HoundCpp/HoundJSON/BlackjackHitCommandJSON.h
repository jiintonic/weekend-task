/* file "BlackjackHitCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKHITCOMMANDJSON_H
#define BLACKJACKHITCOMMANDJSON_H
...
class BlackjackHitCommandJSON : public BlackjackCommandJSON
  {
  public:
      ...
      public:
        ...
        class Generator : public JSONObjectGenerator
          ...
          private:
            ...
            JSONHoldingGenerator<JSONIntegerRangeGenerator<uint8_t, 2, 26>, uint8_t, uint8_t > fieldGeneratorDealerHandTotal;
  ...
  private:
    ...
    BlackjackHitCommandJSON(const BlackjackHitCommandJSON &);
    BlackjackHitCommandJSON & operator=(const BlackjackHitCommandJSON &other);
  ...
  public:
    BlackjackHitCommandJSON(void);
    virtual ~BlackjackHitCommandJSON(void);
    ...
    virtual size_t extraBlackjackHitCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackHitCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackHitCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackHitCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackHitCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraBlackjackHitCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBlackjackHitCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraBlackjackHitCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackHitCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackHitCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackHitCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackHitCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BlackjackHitCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackHitCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackHitCommandJSON>, BlackjackHitCommandJSON *, bool> generator("Type BlackjackHitCommand", ignore_extras);
    ...
    class Generator : public BlackjackCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackCommandJSONKey().c_str(), "BlackjackHitCommand") == 0))
                throw("The key field has a value other than `BlackjackHitCommand'.");
            BlackjackHitCommandJSON *result = new BlackjackHitCommandJSON();
            assert(result != NULL);
            RCHandle<BlackjackHitCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackHitCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(BlackjackHitCommandJSON *result)
          {
        ...
        virtual void handle_result(BlackjackHitCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BlackjackHitCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKHITCOMMANDJSON_H */
