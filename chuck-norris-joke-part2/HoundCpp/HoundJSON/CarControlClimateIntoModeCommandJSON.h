/* file "CarControlClimateIntoModeCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef CARCONTROLCLIMATEINTOMODECOMMANDJSON_H
#define CARCONTROLCLIMATEINTOMODECOMMANDJSON_H
...
class CarControlClimateIntoModeCommandJSON : public CarControlCommandJSON
  {
  public:
  ...
  private:
    ...
    CarControlClimateIntoModeCommandJSON(const CarControlClimateIntoModeCommandJSON &);
    CarControlClimateIntoModeCommandJSON & operator=(const CarControlClimateIntoModeCommandJSON &other);
  ...
  public:
    CarControlClimateIntoModeCommandJSON(void);
    virtual ~CarControlClimateIntoModeCommandJSON(void);
    ...
    virtual size_t extraCarControlClimateIntoModeCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraCarControlClimateIntoModeCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraCarControlClimateIntoModeCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraCarControlClimateIntoModeCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraCarControlClimateIntoModeCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraCarControlClimateIntoModeCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraCarControlClimateIntoModeCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraCarControlClimateIntoModeCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static CarControlClimateIntoModeCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static CarControlClimateIntoModeCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        CarControlClimateIntoModeCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for CarControlClimateIntoModeCommandJSON", &generator);
    ...
    static CarControlClimateIntoModeCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        CarControlClimateIntoModeCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<CarControlClimateIntoModeCommandJSON>, CarControlClimateIntoModeCommandJSON *, bool> generator("Type CarControlClimateIntoModeCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getCarControlCommandJSONKey().c_str(), "CarControlClimateIntoModeCommand") == 0))
                throw("The key field has a value other than `CarControlClimateIntoModeCommand'.");
            CarControlClimateIntoModeCommandJSON *result = new CarControlClimateIntoModeCommandJSON();
            assert(result != NULL);
            RCHandle<CarControlClimateIntoModeCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraCarControlClimateIntoModeCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(CarControlClimateIntoModeCommandJSON *result)
          {
        ...
        virtual void handle_result(CarControlClimateIntoModeCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : CarControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the CarControlClimateIntoModeCommand class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* CARCONTROLCLIMATEINTOMODECOMMANDJSON_H */
