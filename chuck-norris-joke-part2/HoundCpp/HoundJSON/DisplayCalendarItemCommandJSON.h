/* file "DisplayCalendarItemCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef DISPLAYCALENDARITEMCOMMANDJSON_H
#define DISPLAYCALENDARITEMCOMMANDJSON_H
...
class DisplayCalendarItemCommandJSON : public CalendarCommandJSON
  {
  public:
  ...
  private:
    ...
    DisplayCalendarItemCommandJSON(const DisplayCalendarItemCommandJSON &);
    DisplayCalendarItemCommandJSON & operator=(const DisplayCalendarItemCommandJSON &other);
  ...
  public:
    DisplayCalendarItemCommandJSON(void);
    virtual ~DisplayCalendarItemCommandJSON(void);
    ...
    virtual size_t extraDisplayCalendarItemCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraDisplayCalendarItemCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraDisplayCalendarItemCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraDisplayCalendarItemCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraDisplayCalendarItemCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraDisplayCalendarItemCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraDisplayCalendarItemCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraDisplayCalendarItemCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static DisplayCalendarItemCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static DisplayCalendarItemCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        DisplayCalendarItemCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for DisplayCalendarItemCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static DisplayCalendarItemCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        DisplayCalendarItemCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<DisplayCalendarItemCommandJSON>, DisplayCalendarItemCommandJSON *, bool> generator("Type DisplayCalendarItemCommand", ignore_extras);
    ...
    class Generator : public CalendarCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getCalendarCommandJSONKey().c_str(), "DisplayItem") == 0))
                throw("The key field has a value other than `DisplayItem'.");
            DisplayCalendarItemCommandJSON *result = new DisplayCalendarItemCommandJSON();
            assert(result != NULL);
            RCHandle<DisplayCalendarItemCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraDisplayCalendarItemCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(DisplayCalendarItemCommandJSON *result)
          {
            if (fieldGeneratorEventIDs.have_value)
              {
                result->initEventIDs();
        ...
        virtual void handle_result(DisplayCalendarItemCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            switch ((unsigned char)(field_name[0]))
              {
      ...
      public:
        Generator(bool ignore_extras = false) : CalendarCommandJSON::Generator(ignore_extras), fieldGeneratorEventIDs("field \"EventIDs\" of the DisplayCalendarItemCommand class"), fieldGeneratorQuery("field \"Query\" of the DisplayCalendarItemCommand class", ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the DisplayCalendarItemCommand class", ignore_extras), fieldGeneratorCalendarPreferenceIsNotSetResult("field \"CalendarPreferenceIsNotSetResult\" of the DisplayCalendarItemCommand class", ignore_extras), fieldGeneratorNoMatchResult("field \"NoMatchResult\" of the DisplayCalendarItemCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* DISPLAYCALENDARITEMCOMMANDJSON_H */
