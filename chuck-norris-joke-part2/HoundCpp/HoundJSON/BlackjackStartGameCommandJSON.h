/* file "BlackjackStartGameCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKSTARTGAMECOMMANDJSON_H
#define BLACKJACKSTARTGAMECOMMANDJSON_H
...
class BlackjackStartGameCommandJSON : public BlackjackCommandJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      public:
        ...
        class Generator : public JSONObjectGenerator
          ...
          public:
            Generator(bool ignore_extras = false) : fieldGeneratorImageURL("field \"ImageURL\" of the TypeNativeData class"), fieldGeneratorBankroll("field \"Bankroll\" of the TypeNativeData class"), unknownFieldGenerator(ignore_extras)
              {
  ...
  private:
    ...
    BlackjackStartGameCommandJSON(const BlackjackStartGameCommandJSON &);
    BlackjackStartGameCommandJSON & operator=(const BlackjackStartGameCommandJSON &other);
  ...
  public:
    BlackjackStartGameCommandJSON(void);
    virtual ~BlackjackStartGameCommandJSON(void);
    ...
    virtual size_t extraBlackjackStartGameCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackStartGameCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackStartGameCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackStartGameCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackStartGameCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraBlackjackStartGameCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBlackjackStartGameCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraBlackjackStartGameCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackStartGameCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackStartGameCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackStartGameCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackStartGameCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BlackjackStartGameCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackStartGameCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackStartGameCommandJSON>, BlackjackStartGameCommandJSON *, bool> generator("Type BlackjackStartGameCommand", ignore_extras);
    ...
    class Generator : public BlackjackCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackCommandJSONKey().c_str(), "BlackjackStartGameCommand") == 0))
                throw("The key field has a value other than `BlackjackStartGameCommand'.");
            BlackjackStartGameCommandJSON *result = new BlackjackStartGameCommandJSON();
            assert(result != NULL);
            RCHandle<BlackjackStartGameCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackStartGameCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(BlackjackStartGameCommandJSON *result)
          {
        ...
        virtual void handle_result(BlackjackStartGameCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BlackjackStartGameCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKSTARTGAMECOMMANDJSON_H */
