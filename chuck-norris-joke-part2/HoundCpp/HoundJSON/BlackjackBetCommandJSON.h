/* file "BlackjackBetCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKBETCOMMANDJSON_H
#define BLACKJACKBETCOMMANDJSON_H
...
class BlackjackBetCommandJSON : public BlackjackCommandJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      private:
        ...
        bool flagHasPlayerHasBlackjack;
        bool storePlayerHasBlackjack;
        bool flagHasDealerHasBlackjack;
        bool storeDealerHasBlackjack;
        ...
        void  fromJSONPlayerHasBlackjack(JSONValue *json_value, bool ignore_extras = false);

        void  fromJSONDealerHasBlackjack(JSONValue *json_value, bool ignore_extras = false);
      ...
      public:
        ...
        bool  hasPlayerHasBlackjack(void) const;
        bool  getPlayerHasBlackjack(void);
        const bool  getPlayerHasBlackjack(void) const;
        bool  hasDealerHasBlackjack(void) const;
        bool  getDealerHasBlackjack(void);
        const bool  getDealerHasBlackjack(void) const;
        ...
        void setDealerHandTotal(uint8_t new_value)
          {
            ...
            if (new_value > 21)
                throw("The value for field DealerHandTotal of TypeNativeDataJSON is greater than the upper bound (21) for that field.");
        ...
        class Generator : public JSONObjectGenerator
          ...
          private:
            ...
            JSONHoldingGenerator<JSONIntegerRangeGenerator<uint8_t, 2, 21>, uint8_t, uint8_t > fieldGeneratorDealerHandTotal;
            JSONHoldingGenerator<JSONBooleanGenerator, bool, bool > fieldGeneratorPlayerHasBlackjack;
            JSONHoldingGenerator<JSONBooleanGenerator, bool, bool > fieldGeneratorDealerHasBlackjack;
                ...
                switch ((unsigned char)(field_name[0]))
                  {
                    case 'B':
                        ...
                        break;
                    case 'D':
                        if (strncmp(&(field_name[1]), "ealerHa", 7) == 0)
                          {
                            switch ((unsigned char)(field_name[8]))
                              {
                                case 'n':
                                    if (strncmp(&(field_name[9]), "d", 1) == 0)
                                      {
                                    ...
                                    break;
                                case 's':
                                    if (strcmp(&(field_name[9]), "Blackjack") == 0)
                    ...
                    case 'I':
                        ...
                        break;
                    case 'P':
                        if (strncmp(&(field_name[1]), "layerHa", 7) == 0)
                          {
                            switch ((unsigned char)(field_name[8]))
                              {
                                case 'n':
  ...
  private:
    ...
    BlackjackBetCommandJSON(const BlackjackBetCommandJSON &);
    BlackjackBetCommandJSON & operator=(const BlackjackBetCommandJSON &other);
  ...
  public:
    BlackjackBetCommandJSON(void);
    virtual ~BlackjackBetCommandJSON(void);
    ...
    virtual size_t extraBlackjackBetCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackBetCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackBetCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackBetCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackBetCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraBlackjackBetCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBlackjackBetCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraBlackjackBetCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackBetCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackBetCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackBetCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackBetCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BlackjackBetCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackBetCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackBetCommandJSON>, BlackjackBetCommandJSON *, bool> generator("Type BlackjackBetCommand", ignore_extras);
    ...
    class Generator : public BlackjackCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackCommandJSONKey().c_str(), "BlackjackBetCommand") == 0))
                throw("The key field has a value other than `BlackjackBetCommand'.");
            BlackjackBetCommandJSON *result = new BlackjackBetCommandJSON();
            assert(result != NULL);
            RCHandle<BlackjackBetCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackBetCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(BlackjackBetCommandJSON *result)
          {
        ...
        virtual void handle_result(BlackjackBetCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BlackjackBetCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKBETCOMMANDJSON_H */
