/* file "BatteryCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BATTERYCOMMANDJSON_H
#define BATTERYCOMMANDJSON_H
...
class BatteryCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    BatteryCommandJSON(const BatteryCommandJSON &);
    BatteryCommandJSON & operator=(const BatteryCommandJSON &other);
  ...
  public:
    BatteryCommandJSON(void);
    virtual ~BatteryCommandJSON(void);
    ...
    virtual size_t extraBatteryCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBatteryCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBatteryCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBatteryCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBatteryCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraBatteryCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBatteryCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraBatteryCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BatteryCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BatteryCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BatteryCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BatteryCommandJSON", &generator);
    ...
    static BatteryCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BatteryCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BatteryCommandJSON>, BatteryCommandJSON *, bool> generator("Type BatteryCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "BatteryCommand") == 0))
                throw("The key field has a value other than `BatteryCommand'.");
            BatteryCommandJSON *result = new BatteryCommandJSON();
            assert(result != NULL);
            RCHandle<BatteryCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBatteryCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(BatteryCommandJSON *result)
          {
        ...
        virtual void handle_result(BatteryCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the BatteryCommand class"), unknownFieldGenerator(ignore_extras)
          {
