/* file "PreviousPresetOrChannelCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef PREVIOUSPRESETORCHANNELCOMMANDJSON_H
#define PREVIOUSPRESETORCHANNELCOMMANDJSON_H
...
class PreviousPresetOrChannelCommandJSON : public RadioCommandJSON
  {
  private:
    ...
    PreviousPresetOrChannelCommandJSON(const PreviousPresetOrChannelCommandJSON &);
    PreviousPresetOrChannelCommandJSON & operator=(const PreviousPresetOrChannelCommandJSON &other);
  ...
  public:
    PreviousPresetOrChannelCommandJSON(void);
    virtual ~PreviousPresetOrChannelCommandJSON(void);
    ...
    virtual size_t extraPreviousPresetOrChannelCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraPreviousPresetOrChannelCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraPreviousPresetOrChannelCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraPreviousPresetOrChannelCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraPreviousPresetOrChannelCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraPreviousPresetOrChannelCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraPreviousPresetOrChannelCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraPreviousPresetOrChannelCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static PreviousPresetOrChannelCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static PreviousPresetOrChannelCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        PreviousPresetOrChannelCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for PreviousPresetOrChannelCommandJSON", &generator);
    ...
    static PreviousPresetOrChannelCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        PreviousPresetOrChannelCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<PreviousPresetOrChannelCommandJSON>, PreviousPresetOrChannelCommandJSON *, bool> generator("Type PreviousPresetOrChannelCommand", ignore_extras);
    ...
    class Generator : public RadioCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getRadioCommandJSONKey().c_str(), "PreviousPresetOrChannel") == 0))
                throw("The key field has a value other than `PreviousPresetOrChannel'.");
            PreviousPresetOrChannelCommandJSON *result = new PreviousPresetOrChannelCommandJSON();
            assert(result != NULL);
            RCHandle<PreviousPresetOrChannelCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraPreviousPresetOrChannelCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(PreviousPresetOrChannelCommandJSON *result)
          {
        ...
        virtual void handle_result(PreviousPresetOrChannelCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : RadioCommandJSON::Generator(ignore_extras), fieldGeneratorPresetResult("field \"PresetResult\" of the PreviousPresetOrChannelCommand class", ignore_extras), fieldGeneratorChannelResult("field \"ChannelResult\" of the PreviousPresetOrChannelCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* PREVIOUSPRESETORCHANNELCOMMANDJSON_H */
