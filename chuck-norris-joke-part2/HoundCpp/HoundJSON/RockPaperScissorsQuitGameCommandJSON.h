/* file "RockPaperScissorsQuitGameCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef ROCKPAPERSCISSORSQUITGAMECOMMANDJSON_H
#define ROCKPAPERSCISSORSQUITGAMECOMMANDJSON_H
...
class RockPaperScissorsQuitGameCommandJSON : public RockPaperScissorsCommandJSON
  {
  public:
  ...
  private:
    ...
    RockPaperScissorsQuitGameCommandJSON(const RockPaperScissorsQuitGameCommandJSON &);
    RockPaperScissorsQuitGameCommandJSON & operator=(const RockPaperScissorsQuitGameCommandJSON &other);
  ...
  public:
    RockPaperScissorsQuitGameCommandJSON(void);
    virtual ~RockPaperScissorsQuitGameCommandJSON(void);
    ...
    virtual size_t extraRockPaperScissorsQuitGameCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraRockPaperScissorsQuitGameCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraRockPaperScissorsQuitGameCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraRockPaperScissorsQuitGameCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraRockPaperScissorsQuitGameCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraRockPaperScissorsQuitGameCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraRockPaperScissorsQuitGameCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraRockPaperScissorsQuitGameCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static RockPaperScissorsQuitGameCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static RockPaperScissorsQuitGameCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        RockPaperScissorsQuitGameCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for RockPaperScissorsQuitGameCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static RockPaperScissorsQuitGameCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        RockPaperScissorsQuitGameCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<RockPaperScissorsQuitGameCommandJSON>, RockPaperScissorsQuitGameCommandJSON *, bool> generator("Type RockPaperScissorsQuitGameCommand", ignore_extras);
    ...
    class Generator : public RockPaperScissorsCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getRockPaperScissorsCommandJSONKey().c_str(), "RockPaperScissorsQuitCommand") == 0))
            ...
            assert(result != NULL);
            RCHandle<RockPaperScissorsQuitGameCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraRockPaperScissorsQuitGameCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(RockPaperScissorsQuitGameCommandJSON *result)
          {
        ...
        virtual void handle_result(RockPaperScissorsQuitGameCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : RockPaperScissorsCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the RockPaperScissorsQuitGameCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* ROCKPAPERSCISSORSQUITGAMECOMMANDJSON_H */
