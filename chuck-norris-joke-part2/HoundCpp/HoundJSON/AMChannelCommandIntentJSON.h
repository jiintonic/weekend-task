/* file "AMChannelCommandIntentJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef AMCHANNELCOMMANDINTENTJSON_H
#define AMCHANNELCOMMANDINTENTJSON_H
...
class AMChannelCommandIntentJSON : public RadioCommandIntentJSON
  {
  private:
    bool flagHasChannel;
    ...
    AMChannelCommandIntentJSON(const AMChannelCommandIntentJSON &);
    AMChannelCommandIntentJSON & operator=(const AMChannelCommandIntentJSON &other);
  ...
  public:
    AMChannelCommandIntentJSON(void);
    virtual ~AMChannelCommandIntentJSON(void);
    ...
    virtual size_t extraAMChannelCommandIntentComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraAMChannelCommandIntentComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraAMChannelCommandIntentComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraAMChannelCommandIntentComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraAMChannelCommandIntentLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraAMChannelCommandIntentLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    void setChannel(uint16_t new_value)
      {
        flagHasChannel = true;
        if (new_value < 530)
            throw("The value for field Channel of AMChannelCommandIntentJSON is less than the lower bound (530) for that field.");
        if (new_value > 1710)
            throw("The value for field Channel of AMChannelCommandIntentJSON is greater than the upper bound (1710) for that field.");
    ...
    virtual void extraAMChannelCommandIntentAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraAMChannelCommandIntentSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static AMChannelCommandIntentJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static AMChannelCommandIntentJSON *from_text(const char *text, bool ignore_extras = false)
      {
        AMChannelCommandIntentJSON *result;
          {
            ...
            parse_json_value(text, "Text for AMChannelCommandIntentJSON", &generator);
    ...
    static AMChannelCommandIntentJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        AMChannelCommandIntentJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<AMChannelCommandIntentJSON>, AMChannelCommandIntentJSON *, bool> generator("Type AMChannelCommandIntent", ignore_extras);
    ...
    class Generator : public RadioCommandIntentJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getRadioCommandIntentJSONKey().c_str(), "AMChannel") == 0))
                throw("The key field has a value other than `AMChannel'.");
            AMChannelCommandIntentJSON *result = new AMChannelCommandIntentJSON();
            assert(result != NULL);
            RCHandle<AMChannelCommandIntentJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraAMChannelCommandIntentAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(AMChannelCommandIntentJSON *result)
          {
        ...
        virtual void handle_result(AMChannelCommandIntentJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : RadioCommandIntentJSON::Generator(ignore_extras), fieldGeneratorChannel("field \"Channel\" of the AMChannelCommandIntent class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* AMCHANNELCOMMANDINTENTJSON_H */
