/* file "BlackjackActionNotSupportedCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKACTIONNOTSUPPORTEDCOMMANDJSON_H
#define BLACKJACKACTIONNOTSUPPORTEDCOMMANDJSON_H
...
class BlackjackActionNotSupportedCommandJSON : public BlackjackCommandJSON
  {
  public:
        ...
        void setPlayerHandTotal(uint8_t new_value)
          {
            ...
            if (new_value > 20)
                throw("The value for field PlayerHandTotal of TypeNativeDataJSON is greater than the upper bound (20) for that field.");
  ...
  private:
    ...
    BlackjackActionNotSupportedCommandJSON(const BlackjackActionNotSupportedCommandJSON &);
    BlackjackActionNotSupportedCommandJSON & operator=(const BlackjackActionNotSupportedCommandJSON &other);
  ...
  public:
    BlackjackActionNotSupportedCommandJSON(void);
    virtual ~BlackjackActionNotSupportedCommandJSON(void);
    ...
    virtual size_t extraBlackjackActionNotSupportedCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackActionNotSupportedCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackActionNotSupportedCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackActionNotSupportedCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackActionNotSupportedCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraBlackjackActionNotSupportedCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBlackjackActionNotSupportedCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraBlackjackActionNotSupportedCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackActionNotSupportedCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackActionNotSupportedCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackActionNotSupportedCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackActionNotSupportedCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BlackjackActionNotSupportedCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackActionNotSupportedCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackActionNotSupportedCommandJSON>, BlackjackActionNotSupportedCommandJSON *, bool> generator("Type BlackjackActionNotSupportedCommand", ignore_extras);
    ...
    class Generator : public BlackjackCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackCommandJSONKey().c_str(), "BlackjackActionNotSupportedCommand") == 0))
                throw("The key field has a value other than `BlackjackActionNotSupportedCommand'.");
            BlackjackActionNotSupportedCommandJSON *result = new BlackjackActionNotSupportedCommandJSON();
            assert(result != NULL);
            RCHandle<BlackjackActionNotSupportedCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackActionNotSupportedCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(BlackjackActionNotSupportedCommandJSON *result)
          {
        ...
        virtual void handle_result(BlackjackActionNotSupportedCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BlackjackActionNotSupportedCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKACTIONNOTSUPPORTEDCOMMANDJSON_H */
