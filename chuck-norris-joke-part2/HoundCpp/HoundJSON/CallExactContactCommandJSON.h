/* file "CallExactContactCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef CALLEXACTCONTACTCOMMANDJSON_H
#define CALLEXACTCONTACTCOMMANDJSON_H
...
class CallExactContactCommandJSON : public PhoneCommandJSON
  {
  private:
    bool flagHasNumber;
    ...
    CallExactContactCommandJSON(const CallExactContactCommandJSON &);
    CallExactContactCommandJSON & operator=(const CallExactContactCommandJSON &other);
  ...
  public:
    CallExactContactCommandJSON(void);
    virtual ~CallExactContactCommandJSON(void);
    ...
    virtual size_t extraCallExactContactCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraCallExactContactCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraCallExactContactCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraCallExactContactCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraCallExactContactCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraCallExactContactCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraCallExactContactCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraCallExactContactCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static CallExactContactCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static CallExactContactCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        CallExactContactCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for CallExactContactCommandJSON", &generator);
    ...
    static CallExactContactCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        CallExactContactCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<CallExactContactCommandJSON>, CallExactContactCommandJSON *, bool> generator("Type CallExactContactCommand", ignore_extras);
    ...
    class Generator : public PhoneCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getPhoneCommandJSONKey().c_str(), "CallExactContact") == 0))
                throw("The key field has a value other than `CallExactContact'.");
            CallExactContactCommandJSON *result = new CallExactContactCommandJSON();
            assert(result != NULL);
            RCHandle<CallExactContactCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraCallExactContactCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(CallExactContactCommandJSON *result)
          {
        ...
        virtual void handle_result(CallExactContactCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : PhoneCommandJSON::Generator(ignore_extras), fieldGeneratorNumber("field \"Number\" of the CallExactContactCommand class"), fieldGeneratorContacts("field \"Contacts\" of the CallExactContactCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* CALLEXACTCONTACTCOMMANDJSON_H */
