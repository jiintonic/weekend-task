/* file "BlackjackStandCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKSTANDCOMMANDJSON_H
#define BLACKJACKSTANDCOMMANDJSON_H
...
class BlackjackStandCommandJSON : public BlackjackCommandJSON
  {
  public:
        ...
        void setDealerHandTotal(uint8_t new_value)
          {
            ...
            if (new_value < 17)
                throw("The value for field DealerHandTotal of TypeNativeDataJSON is less than the lower bound (17) for that field.");
            if (new_value > 26)
  ...
  private:
    ...
    BlackjackStandCommandJSON(const BlackjackStandCommandJSON &);
    BlackjackStandCommandJSON & operator=(const BlackjackStandCommandJSON &other);
  ...
  public:
    BlackjackStandCommandJSON(void);
    virtual ~BlackjackStandCommandJSON(void);
    ...
    virtual size_t extraBlackjackStandCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackStandCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackStandCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackStandCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackStandCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraBlackjackStandCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBlackjackStandCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraBlackjackStandCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackStandCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackStandCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackStandCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackStandCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BlackjackStandCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackStandCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackStandCommandJSON>, BlackjackStandCommandJSON *, bool> generator("Type BlackjackStandCommand", ignore_extras);
    ...
    class Generator : public BlackjackCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackCommandJSONKey().c_str(), "BlackjackStandCommand") == 0))
            ...
            assert(result != NULL);
            RCHandle<BlackjackStandCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackStandCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(BlackjackStandCommandJSON *result)
          {
        ...
        virtual void handle_result(BlackjackStandCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BlackjackStandCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKSTANDCOMMANDJSON_H */
