/* file "VoiceSynthesisCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef VOICESYNTHESISCOMMANDJSON_H
#define VOICESYNTHESISCOMMANDJSON_H
...
class VoiceSynthesisCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    VoiceSynthesisCommandJSON(const VoiceSynthesisCommandJSON &);
    VoiceSynthesisCommandJSON & operator=(const VoiceSynthesisCommandJSON &other);
  ...
  public:
    VoiceSynthesisCommandJSON(void);
    virtual ~VoiceSynthesisCommandJSON(void);
    ...
    virtual size_t extraVoiceSynthesisCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraVoiceSynthesisCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraVoiceSynthesisCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraVoiceSynthesisCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraVoiceSynthesisCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraVoiceSynthesisCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraVoiceSynthesisCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraVoiceSynthesisCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static VoiceSynthesisCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static VoiceSynthesisCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        VoiceSynthesisCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for VoiceSynthesisCommandJSON", &generator);
    ...
    static VoiceSynthesisCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        VoiceSynthesisCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<VoiceSynthesisCommandJSON>, VoiceSynthesisCommandJSON *, bool> generator("Type VoiceSynthesisCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "VoiceSynthesisCommand") == 0))
                throw("The key field has a value other than `VoiceSynthesisCommand'.");
            VoiceSynthesisCommandJSON *result = new VoiceSynthesisCommandJSON();
            assert(result != NULL);
            RCHandle<VoiceSynthesisCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraVoiceSynthesisCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(VoiceSynthesisCommandJSON *result)
          {
        ...
        virtual void handle_result(VoiceSynthesisCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the VoiceSynthesisCommand class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* VOICESYNTHESISCOMMANDJSON_H */
