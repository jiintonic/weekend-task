/* file "BaseConverterCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BASECONVERTERCOMMANDJSON_H
#define BASECONVERTERCOMMANDJSON_H
...
class BaseConverterCommandJSON : public CommandResultJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      public:
        ...
        class Generator : public JSONObjectGenerator
          ...
          public:
            Generator(bool ignore_extras = false) : fieldGeneratorConversionRows("field \"ConversionRows\" of the TypeNativeData class", ignore_extras), fieldGeneratorErrorMessage("field \"ErrorMessage\" of the TypeNativeData class")
              {
  ...
  private:
    ...
    BaseConverterCommandJSON(const BaseConverterCommandJSON &);
    BaseConverterCommandJSON & operator=(const BaseConverterCommandJSON &other);
  ...
  public:
    BaseConverterCommandJSON(void);
    virtual ~BaseConverterCommandJSON(void);
    ...
    virtual size_t extraBaseConverterCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBaseConverterCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBaseConverterCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBaseConverterCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBaseConverterCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraBaseConverterCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBaseConverterCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraBaseConverterCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BaseConverterCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BaseConverterCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BaseConverterCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BaseConverterCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BaseConverterCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BaseConverterCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BaseConverterCommandJSON>, BaseConverterCommandJSON *, bool> generator("Type BaseConverterCommand", ignore_extras);
    ...
    class Generator : public CommandResultJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getCommandResultJSONKey().c_str(), "BaseConverterCommand") == 0))
                throw("The key field has a value other than `BaseConverterCommand'.");
            BaseConverterCommandJSON *result = new BaseConverterCommandJSON();
            assert(result != NULL);
            RCHandle<BaseConverterCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBaseConverterCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(BaseConverterCommandJSON *result)
          {
        ...
        virtual void handle_result(BaseConverterCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : CommandResultJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BaseConverterCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BASECONVERTERCOMMANDJSON_H */
