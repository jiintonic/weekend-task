/* file "RobotCommandJSON.cpp" */

/* Generated automatically by Classy JSON. */


#pragma implementation "RobotCommandJSON.h"

#include "RobotCommandJSON.h"
#include "RobotGoToRoomCommandJSON.h"
#include "RobotListCapabilitiesCommandJSON.h"
#include "RobotListKnownRoomsCommandJSON.h"
#include "RobotMoveCommandJSON.h"
#include "RobotSetKnownRoomsCommandJSON.h"
#include "RobotStopCommandJSON.h"
#include "RobotStopRecordingVideoCommandJSON.h"
#include "RobotTakePictureCommandJSON.h"
#include "RobotTakeVideoCommandJSON.h"
#include "RobotTurnCommandJSON.h"


RobotCommandJSON::RobotCommandJSON(const RobotCommandJSON &)
  {
    assert(false);
  }

RobotCommandJSON &RobotCommandJSON::operator=(const RobotCommandJSON &other)
  {
    assert(false);
    throw "Illegal operator=() call.";
  }

RobotCommandJSON::RobotCommandJSON(void)
  {
  }

RobotCommandJSON::~RobotCommandJSON(void)
  {
  }

const char *RobotCommandJSON::getCommandKind(void) const
  {
    return "RobotCommand";
  }

RobotCommandJSON *RobotCommandJSON::createForKey(const char *key, string_index *other_field_index)
  {
    switch ((unsigned char)(key[0]))
      {
        case 'G':
            if (strcmp(&(key[1]), "oToRoomCommand") == 0)
                return new RobotGoToRoomCommandJSON();
            break;
        case 'L':
            if (strncmp(&(key[1]), "ist", 3) == 0)
              {
                switch ((unsigned char)(key[4]))
                  {
                    case 'C':
                        if (strcmp(&(key[5]), "apabilitiesCommand") == 0)
                            return new RobotListCapabilitiesCommandJSON();
                        break;
                    case 'K':
                        if (strcmp(&(key[5]), "nownRoomsCommand") == 0)
                            return new RobotListKnownRoomsCommandJSON();
                        break;
                    default:
                        break;
                  }
              }
            break;
        case 'M':
            if (strcmp(&(key[1]), "oveCommand") == 0)
                return new RobotMoveCommandJSON();
            break;
        case 'R':
            if (strcmp(&(key[1]), "obotSetKnownRoomsCommand") == 0)
                return new RobotSetKnownRoomsCommandJSON();
            break;
        case 'S':
            if (strncmp(&(key[1]), "top", 3) == 0)
              {
                switch ((unsigned char)(key[4]))
                  {
                    case 'C':
                        if (strcmp(&(key[5]), "ommand") == 0)
                            return new RobotStopCommandJSON();
                        break;
                    case 'R':
                        if (strcmp(&(key[5]), "ecordingVideoCommand") == 0)
                            return new RobotStopRecordingVideoCommandJSON();
                        break;
                    default:
                        break;
                  }
              }
            break;
        case 'T':
            switch ((unsigned char)(key[1]))
              {
                case 'a':
                    if (strncmp(&(key[2]), "ke", 2) == 0)
                      {
                        switch ((unsigned char)(key[4]))
                          {
                            case 'P':
                                if (strcmp(&(key[5]), "ictureCommand") == 0)
                                    return new RobotTakePictureCommandJSON();
                                break;
                            case 'V':
                                if (strcmp(&(key[5]), "ideoCommand") == 0)
                                    return new RobotTakeVideoCommandJSON();
                                break;
                            default:
                                break;
                          }
                      }
                    break;
                case 'u':
                    if (strcmp(&(key[2]), "rnCommand") == 0)
                        return new RobotTurnCommandJSON();
                    break;
                default:
                    break;
              }
            break;
        default:
            break;
      }

    class GenericRobotCommandJSON : public RobotCommandJSON
      {
      private:
        std::string key;
        std::vector<std::string> extraKeys;
        std::vector<JSONValue *> extraValues;
        string_index *extraIndex;

      public:
        GenericRobotCommandJSON(const char *init_key) : key(init_key)
          {
        extraIndex = create_string_index();
          }
        ~GenericRobotCommandJSON(void)
          {
            size_t extra_count = extraValues.size();
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
                extraValues[extra_num]->remove_reference();
            destroy_string_index(extraIndex);
          }

        const char *getRobotCommandKind(void) const  { return key.c_str(); }
        size_t extraRobotCommandComponentCount(void) const
          {
            assert(extraKeys.size() == extraValues.size());
            return extraKeys.size();
          }
        const char *extraRobotCommandComponentKey(size_t component_num) const
          {
            assert(extraKeys.size() == extraValues.size());
            assert(component_num < extraValues.size());
            return extraKeys[component_num].c_str();
          }
        JSONValue *extraRobotCommandComponentValue(size_t component_num)
          {
            assert(extraKeys.size() == extraValues.size());
            assert(component_num < extraValues.size());
            return extraValues[component_num];
          }
        const JSONValue *extraRobotCommandComponentValue(size_t component_num) const
          {
            assert(extraKeys.size() == extraValues.size());
            assert(component_num < extraValues.size());
            return extraValues[component_num];
          }
        JSONValue *extraRobotCommandLookup(const char *field_name)
          {
            JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
            if (result != NULL)
                result->add_reference();
            return result;
          }
        const JSONValue *extraRobotCommandLookup(const char *field_name) const
          {
            JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
            if (result != NULL)
                result->add_reference();
            return result;
          }
        void extraRobotCommandAppendPair(const char *key, JSONValue *new_component)
          {
            assert(key != NULL);
            assert(new_component != NULL);

            assert(extraKeys.size() == extraValues.size());
            extraKeys.push_back(key);
            extraValues.push_back(new_component);
            enter_into_string_index(extraIndex, key, new_component);
            new_component->add_reference();
          }
        void extraRobotCommandSetField(const char *key, JSONValue *new_component)
          {
            assert(key != NULL);
            assert(new_component != NULL);

            JSONValue *old_field = extraRobotCommandLookup(key);
            if (old_field == NULL)
              {
                extraRobotCommandAppendPair(key, new_component);
              }
            else
              {
                size_t count = extraKeys.size();
                assert(count == extraValues.size());
                for (size_t num = 0; num < count; ++num)
                  {
                    if (strcmp(extraKeys[num].c_str(), key) == 0)
                      {
                        extraValues[num] = new_component;
                        break;
                      }
                  }
                enter_into_string_index(extraIndex, key, new_component);
                new_component->add_reference();
                old_field->remove_reference();
                old_field->remove_reference();
              }
          }

        void write_as_json(JSONHandler *handler) const
          {
            handler->start_object();
            write_fields_as_json(handler);
            size_t extra_count = extraKeys.size();
            assert(extra_count == extraValues.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                handler->start_pair(extraKeys[extra_num].c_str());
                extraValues[extra_num]->write(handler);
              }
            handler->finish_object();
          }
      };
    return new GenericRobotCommandJSON(key);
  }
RobotCommandJSON *RobotCommandJSON::from_json(JSONValue *json_value, bool ignore_extras)
  {
    RobotCommandJSON *result;
      {
        JSONHoldingGenerator<Generator, RCHandle<RobotCommandJSON>, RobotCommandJSON *, bool> generator("Type RobotCommand", ignore_extras);
        json_value->write(&generator);
        assert(generator.have_value);
        result = generator.value.referenced();
        result->add_reference();
      };
    result->remove_reference_no_delete();
    return result;
  }
