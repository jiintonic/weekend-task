/* file "NumberGuessNotUnderstoodCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef NUMBERGUESSNOTUNDERSTOODCOMMANDJSON_H
#define NUMBERGUESSNOTUNDERSTOODCOMMANDJSON_H
...
class NumberGuessNotUnderstoodCommandJSON : public NumberGuessCommandJSON
  {
  public:
        ...
        void setGuessesMade(OInteger new_value)
          {
            ...
            if (new_value < 0)
                throw("The value for field GuessesMade of TypeNativeDataJSON is less than the lower bound (0) for that field.");
  ...
  private:
    ...
    NumberGuessNotUnderstoodCommandJSON(const NumberGuessNotUnderstoodCommandJSON &);
    NumberGuessNotUnderstoodCommandJSON & operator=(const NumberGuessNotUnderstoodCommandJSON &other);
  ...
  public:
    NumberGuessNotUnderstoodCommandJSON(void);
    virtual ~NumberGuessNotUnderstoodCommandJSON(void);
    ...
    virtual size_t extraNumberGuessNotUnderstoodCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraNumberGuessNotUnderstoodCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraNumberGuessNotUnderstoodCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraNumberGuessNotUnderstoodCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraNumberGuessNotUnderstoodCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraNumberGuessNotUnderstoodCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraNumberGuessNotUnderstoodCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraNumberGuessNotUnderstoodCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static NumberGuessNotUnderstoodCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static NumberGuessNotUnderstoodCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        NumberGuessNotUnderstoodCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for NumberGuessNotUnderstoodCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static NumberGuessNotUnderstoodCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        NumberGuessNotUnderstoodCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<NumberGuessNotUnderstoodCommandJSON>, NumberGuessNotUnderstoodCommandJSON *, bool> generator("Type NumberGuessNotUnderstoodCommand", ignore_extras);
    ...
    class Generator : public NumberGuessCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getNumberGuessCommandJSONKey().c_str(), "NumberGuessNotUnderstoodCommand") == 0))
                throw("The key field has a value other than `NumberGuessNotUnderstoodCommand'.");
            NumberGuessNotUnderstoodCommandJSON *result = new NumberGuessNotUnderstoodCommandJSON();
            assert(result != NULL);
            RCHandle<NumberGuessNotUnderstoodCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraNumberGuessNotUnderstoodCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(NumberGuessNotUnderstoodCommandJSON *result)
          {
        ...
        virtual void handle_result(NumberGuessNotUnderstoodCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : NumberGuessCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the NumberGuessNotUnderstoodCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* NUMBERGUESSNOTUNDERSTOODCOMMANDJSON_H */
