/* file "NumberGuessQuitGameCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef NUMBERGUESSQUITGAMECOMMANDJSON_H
#define NUMBERGUESSQUITGAMECOMMANDJSON_H
...
class NumberGuessQuitGameCommandJSON : public NumberGuessCommandJSON
  {
  public:
  ...
  private:
    ...
    NumberGuessQuitGameCommandJSON(const NumberGuessQuitGameCommandJSON &);
    NumberGuessQuitGameCommandJSON & operator=(const NumberGuessQuitGameCommandJSON &other);
  ...
  public:
    NumberGuessQuitGameCommandJSON(void);
    virtual ~NumberGuessQuitGameCommandJSON(void);
    ...
    virtual size_t extraNumberGuessQuitGameCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraNumberGuessQuitGameCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraNumberGuessQuitGameCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraNumberGuessQuitGameCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraNumberGuessQuitGameCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraNumberGuessQuitGameCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraNumberGuessQuitGameCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraNumberGuessQuitGameCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static NumberGuessQuitGameCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static NumberGuessQuitGameCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        NumberGuessQuitGameCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for NumberGuessQuitGameCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static NumberGuessQuitGameCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        NumberGuessQuitGameCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<NumberGuessQuitGameCommandJSON>, NumberGuessQuitGameCommandJSON *, bool> generator("Type NumberGuessQuitGameCommand", ignore_extras);
    ...
    class Generator : public NumberGuessCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getNumberGuessCommandJSONKey().c_str(), "NumberGuessQuitCommand") == 0))
            ...
            assert(result != NULL);
            RCHandle<NumberGuessQuitGameCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraNumberGuessQuitGameCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(NumberGuessQuitGameCommandJSON *result)
          {
        ...
        virtual void handle_result(NumberGuessQuitGameCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : NumberGuessCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the NumberGuessQuitGameCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* NUMBERGUESSQUITGAMECOMMANDJSON_H */
