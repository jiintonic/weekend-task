...
#pragma implementation "UnitJSON.h"

#include "UnitJSON.h"
#include "CoreUnitsUnitJSON.h"


UnitJSON::UnitJSON(const UnitJSON &)
  {
    assert(false);
  }

UnitJSON &UnitJSON::operator=(const UnitJSON &other)
  {
    assert(false);
    throw "Illegal operator=() call.";
  }

UnitJSON::UnitJSON(void)
  {
  }
...
UnitJSON *UnitJSON::createForKey(const char *key, string_index *other_field_index)
  {
    if (strcmp(key, "Core") == 0)
        return new CoreUnitsUnitJSON();

    class GenericUnitJSON : public UnitJSON
      {
      private:
        std::string key;
      ...
      public:
        GenericUnitJSON(const char *init_key) : key(init_key)
          {
        extraIndex = create_string_index();
          }
        ~GenericUnitJSON(void)
          {
        ...
        const char *getUnitKind(void) const  { return key.c_str(); }
...
UnitJSON *UnitJSON::from_json(JSONValue *json_value, bool ignore_extras)
  {
    UnitJSON *result;
      {
