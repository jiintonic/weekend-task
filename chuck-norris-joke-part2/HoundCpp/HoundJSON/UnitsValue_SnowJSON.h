/* file "UnitsValue_SnowJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UNITSVALUE_SNOWJSON_H
#define UNITSVALUE_SNOWJSON_H
...
class UnitsValue_SnowJSON : public ReferenceCounted
  {
  private:
    bool flagHasFormat;
    ...
    UnitsValue_SnowJSON(const UnitsValue_SnowJSON &);
    UnitsValue_SnowJSON & operator=(const UnitsValue_SnowJSON &other);
  ...
  public:
    UnitsValue_SnowJSON(void);
    virtual ~UnitsValue_SnowJSON(void);
    ...
    virtual size_t extraUnitsValue_SnowComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUnitsValue_SnowComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUnitsValue_SnowComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUnitsValue_SnowComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUnitsValue_SnowLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraUnitsValue_SnowLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    void setValue(double new_value)
      {
        flagHasValue = true;
        if (new_value < 0)
            throw("The value for field Value of UnitsValue_SnowJSON is less than the lower bound (0) for that field.");
    ...
    void setValueText(std::string new_value)
      {
        ...
        if (!(JSONWriter::is_valid_number_format(new_value.c_str())))
            throw("The text value for field Value of UnitsValue_SnowJSON is not a valid number.");
    ...
    virtual void extraUnitsValue_SnowAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraUnitsValue_SnowSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UnitsValue_SnowJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UnitsValue_SnowJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UnitsValue_SnowJSON *result;
          {
            ...
            parse_json_value(text, "Text for UnitsValue_SnowJSON", &generator);
    ...
    static UnitsValue_SnowJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UnitsValue_SnowJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UnitsValue_SnowJSON>, UnitsValue_SnowJSON *, bool> generator("Type UnitsValue_Snow", ignore_extras);
    ...
    class Generator : public JSONObjectGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            ...
            assert(result != NULL);
            RCHandle<UnitsValue_SnowJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUnitsValue_SnowAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(UnitsValue_SnowJSON *result)
          {
        ...
        virtual void handle_result(UnitsValue_SnowJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : fieldGeneratorFormat("field \"Format\" of the UnitsValue_Snow class"), fieldGeneratorUnits("field \"Units\" of the UnitsValue_Snow class"), fieldGeneratorWrittenUnits("field \"WrittenUnits\" of the UnitsValue_Snow class"), fieldGeneratorSpokenUnits("field \"SpokenUnits\" of the UnitsValue_Snow class"), fieldGeneratorValue("field \"Value\" of the UnitsValue_Snow class"), unknownFieldGenerator(ignore_extras)
          {
