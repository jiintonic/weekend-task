/* file "UberProductsCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UBERPRODUCTSCOMMANDJSON_H
#define UBERPRODUCTSCOMMANDJSON_H
...
class UberProductsCommandJSON : public UberCommandJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      public:
        ...
        class Generator : public JSONObjectGenerator
          ...
          public:
            Generator(bool ignore_extras = false) : fieldGeneratorUberProductsSpec("field \"UberProductsSpec\" of the TypeNativeData class", ignore_extras), fieldGeneratorUberProducts("field \"UberProducts\" of the TypeNativeData class", ignore_extras), unknownFieldGenerator(ignore_extras)
              {
  ...
  private:
    ...
    UberProductsCommandJSON(const UberProductsCommandJSON &);
    UberProductsCommandJSON & operator=(const UberProductsCommandJSON &other);
  ...
  public:
    UberProductsCommandJSON(void);
    virtual ~UberProductsCommandJSON(void);
    ...
    virtual size_t extraUberProductsCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUberProductsCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUberProductsCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUberProductsCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUberProductsCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraUberProductsCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraUberProductsCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraUberProductsCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UberProductsCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UberProductsCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UberProductsCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for UberProductsCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static UberProductsCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UberProductsCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UberProductsCommandJSON>, UberProductsCommandJSON *, bool> generator("Type UberProductsCommand", ignore_extras);
    ...
    class Generator : public UberCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberCommandJSONKey().c_str(), "UberProducts") == 0))
                throw("The key field has a value other than `UberProducts'.");
            UberProductsCommandJSON *result = new UberProductsCommandJSON();
            assert(result != NULL);
            RCHandle<UberProductsCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUberProductsCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(UberProductsCommandJSON *result)
          {
        ...
        virtual void handle_result(UberProductsCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the UberProductsCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* UBERPRODUCTSCOMMANDJSON_H */
