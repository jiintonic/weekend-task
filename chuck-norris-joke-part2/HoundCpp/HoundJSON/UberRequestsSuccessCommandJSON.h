/* file "UberRequestsSuccessCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UBERREQUESTSSUCCESSCOMMANDJSON_H
#define UBERREQUESTSSUCCESSCOMMANDJSON_H
...
class UberRequestsSuccessCommandJSON : public UberRequestsCommandJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      private:
        ...
        bool flagHasUberProduct;
        UberCompositeProductJSON * storeUberProduct;
        ...
        void  fromJSONUberProduct(JSONValue *json_value, bool ignore_extras = false);
      ...
      public:
        ...
        bool  hasUberProduct(void) const;
        UberCompositeProductJSON *  getUberProduct(void);
        const UberCompositeProductJSON *  getUberProduct(void) const;
        ...
        void setUberProduct(UberCompositeProductJSON * new_value)
          {
            new_value->add_reference();
        ...
        class Generator : public JSONObjectGenerator
          ...
          private:
            ...
            JSONHoldingGenerator<UberCompositeProductJSON::Generator, RCHandle<UberCompositeProductJSON>, UberCompositeProductJSON *, bool > fieldGeneratorUberProduct;
            ...
            void finish(TypeNativeDataJSON *result)
              {
                ...
                if (fieldGeneratorUberProduct.have_value)
                  {
                    result->setUberProduct(fieldGeneratorUberProduct.value.referenced());
                    fieldGeneratorUberProduct.have_value = false;
                  }
                else if (!(result->hasUberProduct()))
                  {
                ...
                if (strncmp(field_name, "Uber", 4) == 0)
                  {
                    switch ((unsigned char)(field_name[4]))
                      {
                        case 'P':
                            if (strcmp(&(field_name[5]), "roduct") == 0)
                                return &fieldGeneratorUberProduct;
                            break;
                        case 'R':
                            if (strncmp(&(field_name[5]), "equest", 6) == 0)
                              {
                                switch ((unsigned char)(field_name[11]))
                                  {
                                    ...
                                    case 's':
                                        if (strcmp(&(field_name[12]), "Spec") == 0)
  ...
  private:
    ...
    UberRequestsSuccessCommandJSON(const UberRequestsSuccessCommandJSON &);
    UberRequestsSuccessCommandJSON & operator=(const UberRequestsSuccessCommandJSON &other);
  ...
  public:
    UberRequestsSuccessCommandJSON(void);
    virtual ~UberRequestsSuccessCommandJSON(void);
    ...
    virtual size_t extraUberRequestsSuccessCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUberRequestsSuccessCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUberRequestsSuccessCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUberRequestsSuccessCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUberRequestsSuccessCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraUberRequestsSuccessCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraUberRequestsSuccessCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraUberRequestsSuccessCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UberRequestsSuccessCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UberRequestsSuccessCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UberRequestsSuccessCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for UberRequestsSuccessCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static UberRequestsSuccessCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UberRequestsSuccessCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UberRequestsSuccessCommandJSON>, UberRequestsSuccessCommandJSON *, bool> generator("Type UberRequestsSuccessCommand", ignore_extras);
    ...
    class Generator : public UberRequestsCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberRequestsCommandJSONKey().c_str(), "UberRequestsSuccess") == 0))
                throw("The key field has a value other than `UberRequestsSuccess'.");
            UberRequestsSuccessCommandJSON *result = new UberRequestsSuccessCommandJSON();
            assert(result != NULL);
            RCHandle<UberRequestsSuccessCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUberRequestsSuccessCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(UberRequestsSuccessCommandJSON *result)
          {
        ...
        virtual void handle_result(UberRequestsSuccessCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberRequestsCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the UberRequestsSuccessCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* UBERREQUESTSSUCCESSCOMMANDJSON_H */
