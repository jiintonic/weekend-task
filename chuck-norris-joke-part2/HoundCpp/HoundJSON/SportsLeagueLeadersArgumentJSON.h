/* file "SportsLeagueLeadersArgumentJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef SPORTSLEAGUELEADERSARGUMENTJSON_H
#define SPORTSLEAGUELEADERSARGUMENTJSON_H
...
class SportsLeagueLeadersArgumentJSON : public ReferenceCounted
  {
  public:
    struct TypeArgumentData
      {
        ...
        union
          {
            SportsSeasonJSON * choice0;
            SportsStatsArgumentTypeModifierJSON * choice1;
          } u;
      };
  ...
  private:
    ...
    SportsLeagueLeadersArgumentJSON(const SportsLeagueLeadersArgumentJSON &);
    SportsLeagueLeadersArgumentJSON & operator=(const SportsLeagueLeadersArgumentJSON &other);
  ...
  public:
    SportsLeagueLeadersArgumentJSON(void);
    virtual ~SportsLeagueLeadersArgumentJSON(void);
    ...
    virtual size_t extraSportsLeagueLeadersArgumentComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraSportsLeagueLeadersArgumentComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraSportsLeagueLeadersArgumentComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraSportsLeagueLeadersArgumentComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraSportsLeagueLeadersArgumentLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraSportsLeagueLeadersArgumentLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    void setArgumentID(OInteger new_value)
      {
        ...
        if (new_value < 0)
            throw("The value for field ArgumentID of SportsLeagueLeadersArgumentJSON is less than the lower bound (0) for that field.");
    ...
    virtual void extraSportsLeagueLeadersArgumentAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraSportsLeagueLeadersArgumentSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);

        JSONValue *old_field = extraSportsLeagueLeadersArgumentLookup(key);
        if (old_field == NULL)
          {
            extraSportsLeagueLeadersArgumentAppendPair(key, new_component);
          }
        else
          {
    ...
    static SportsLeagueLeadersArgumentJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static SportsLeagueLeadersArgumentJSON *from_text(const char *text, bool ignore_extras = false)
      {
        SportsLeagueLeadersArgumentJSON *result;
          {
            ...
            parse_json_value(text, "Text for SportsLeagueLeadersArgumentJSON", &generator);
    ...
    static SportsLeagueLeadersArgumentJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        SportsLeagueLeadersArgumentJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<SportsLeagueLeadersArgumentJSON>, SportsLeagueLeadersArgumentJSON *, bool> generator("Type SportsLeagueLeadersArgument", ignore_extras);
    ...
    class FieldGeneratorArgumentData : public JSONParallelGenerator
          {
          private:
            JSONHoldingGenerator<SportsSeasonJSON::Generator, RCHandle<SportsSeasonJSON>, SportsSeasonJSON *, bool > field0;
            JSONHoldingGenerator<SportsStatsArgumentTypeModifierJSON::Generator, RCHandle<SportsStatsArgumentTypeModifierJSON>, SportsStatsArgumentTypeModifierJSON *, bool > field1;
      ...
      protected:
        ...
        void finish(void)
          {
            SportsLeagueLeadersArgumentJSON *result = new SportsLeagueLeadersArgumentJSON();
            assert(result != NULL);
            RCHandle<SportsLeagueLeadersArgumentJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraSportsLeagueLeadersArgumentAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(SportsLeagueLeadersArgumentJSON *result)
          {
        ...
        virtual void handle_result(SportsLeagueLeadersArgumentJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : fieldGeneratorArgumentData("field \"ArgumentData\" of the SportsLeagueLeadersArgument class", ignore_extras), fieldGeneratorArgumentID("field \"ArgumentID\" of the SportsLeagueLeadersArgument class"), unknownFieldGenerator(ignore_extras)
          {
            set_what("the SportsLeagueLeadersArgument class");
          }
        ~Generator(void)
          {
          }
...
#endif /* SPORTSLEAGUELEADERSARGUMENTJSON_H */
