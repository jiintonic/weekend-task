/* file "UnitsValue_TemperatureJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UNITSVALUE_TEMPERATUREJSON_H
#define UNITSVALUE_TEMPERATUREJSON_H
...
class UnitsValue_TemperatureJSON : public ReferenceCounted
  {
  private:
    bool flagHasFormat;
    ...
    UnitsValue_TemperatureJSON(const UnitsValue_TemperatureJSON &);
    UnitsValue_TemperatureJSON & operator=(const UnitsValue_TemperatureJSON &other);
  ...
  public:
    UnitsValue_TemperatureJSON(void);
    virtual ~UnitsValue_TemperatureJSON(void);
    ...
    virtual size_t extraUnitsValue_TemperatureComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUnitsValue_TemperatureComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUnitsValue_TemperatureComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUnitsValue_TemperatureComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUnitsValue_TemperatureLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraUnitsValue_TemperatureLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraUnitsValue_TemperatureAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraUnitsValue_TemperatureSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UnitsValue_TemperatureJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UnitsValue_TemperatureJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UnitsValue_TemperatureJSON *result;
          {
            ...
            parse_json_value(text, "Text for UnitsValue_TemperatureJSON", &generator);
    ...
    static UnitsValue_TemperatureJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UnitsValue_TemperatureJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UnitsValue_TemperatureJSON>, UnitsValue_TemperatureJSON *, bool> generator("Type UnitsValue_Temperature", ignore_extras);
    ...
    class Generator : public JSONObjectGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            ...
            assert(result != NULL);
            RCHandle<UnitsValue_TemperatureJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUnitsValue_TemperatureAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(UnitsValue_TemperatureJSON *result)
          {
        ...
        virtual void handle_result(UnitsValue_TemperatureJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : fieldGeneratorFormat("field \"Format\" of the UnitsValue_Temperature class"), fieldGeneratorUnits("field \"Units\" of the UnitsValue_Temperature class"), fieldGeneratorWrittenUnits("field \"WrittenUnits\" of the UnitsValue_Temperature class"), fieldGeneratorSpokenUnits("field \"SpokenUnits\" of the UnitsValue_Temperature class"), fieldGeneratorValue("field \"Value\" of the UnitsValue_Temperature class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* UNITSVALUE_TEMPERATUREJSON_H */
