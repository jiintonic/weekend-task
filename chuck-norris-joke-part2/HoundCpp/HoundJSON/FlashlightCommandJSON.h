/* file "FlashlightCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef FLASHLIGHTCOMMANDJSON_H
#define FLASHLIGHTCOMMANDJSON_H
...
class FlashlightCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    FlashlightCommandJSON(const FlashlightCommandJSON &);
    FlashlightCommandJSON & operator=(const FlashlightCommandJSON &other);
  ...
  public:
    FlashlightCommandJSON(void);
    virtual ~FlashlightCommandJSON(void);
    ...
    virtual size_t extraFlashlightCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraFlashlightCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraFlashlightCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraFlashlightCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraFlashlightCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraFlashlightCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraFlashlightCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraFlashlightCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static FlashlightCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static FlashlightCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        FlashlightCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for FlashlightCommandJSON", &generator);
    ...
    static FlashlightCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        FlashlightCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<FlashlightCommandJSON>, FlashlightCommandJSON *, bool> generator("Type FlashlightCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "FlashlightCommand") == 0))
                throw("The key field has a value other than `FlashlightCommand'.");
            FlashlightCommandJSON *result = new FlashlightCommandJSON();
            assert(result != NULL);
            RCHandle<FlashlightCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraFlashlightCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(FlashlightCommandJSON *result)
          {
        ...
        virtual void handle_result(FlashlightCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the FlashlightCommand class"), unknownFieldGenerator(ignore_extras)
          {
