/* file "NumberGuessSetDifficultyCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef NUMBERGUESSSETDIFFICULTYCOMMANDJSON_H
#define NUMBERGUESSSETDIFFICULTYCOMMANDJSON_H
...
class NumberGuessSetDifficultyCommandJSON : public NumberGuessCommandJSON
  {
  public:
  ...
  private:
    ...
    NumberGuessSetDifficultyCommandJSON(const NumberGuessSetDifficultyCommandJSON &);
    NumberGuessSetDifficultyCommandJSON & operator=(const NumberGuessSetDifficultyCommandJSON &other);
  ...
  public:
    NumberGuessSetDifficultyCommandJSON(void);
    virtual ~NumberGuessSetDifficultyCommandJSON(void);
    ...
    virtual size_t extraNumberGuessSetDifficultyCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraNumberGuessSetDifficultyCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraNumberGuessSetDifficultyCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraNumberGuessSetDifficultyCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraNumberGuessSetDifficultyCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraNumberGuessSetDifficultyCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraNumberGuessSetDifficultyCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraNumberGuessSetDifficultyCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static NumberGuessSetDifficultyCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static NumberGuessSetDifficultyCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        NumberGuessSetDifficultyCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for NumberGuessSetDifficultyCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static NumberGuessSetDifficultyCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        NumberGuessSetDifficultyCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<NumberGuessSetDifficultyCommandJSON>, NumberGuessSetDifficultyCommandJSON *, bool> generator("Type NumberGuessSetDifficultyCommand", ignore_extras);
    ...
    class Generator : public NumberGuessCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getNumberGuessCommandJSONKey().c_str(), "NumberGuessSetDifficultyCommand") == 0))
                throw("The key field has a value other than `NumberGuessSetDifficultyCommand'.");
            NumberGuessSetDifficultyCommandJSON *result = new NumberGuessSetDifficultyCommandJSON();
            assert(result != NULL);
            RCHandle<NumberGuessSetDifficultyCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraNumberGuessSetDifficultyCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(NumberGuessSetDifficultyCommandJSON *result)
          {
        ...
        virtual void handle_result(NumberGuessSetDifficultyCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : NumberGuessCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the NumberGuessSetDifficultyCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* NUMBERGUESSSETDIFFICULTYCOMMANDJSON_H */
