/* file "WifiCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef WIFICOMMANDJSON_H
#define WIFICOMMANDJSON_H
...
class WifiCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    WifiCommandJSON(const WifiCommandJSON &);
    WifiCommandJSON & operator=(const WifiCommandJSON &other);
  ...
  public:
    WifiCommandJSON(void);
    virtual ~WifiCommandJSON(void);
    ...
    virtual size_t extraWifiCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraWifiCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraWifiCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraWifiCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraWifiCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraWifiCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraWifiCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraWifiCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static WifiCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static WifiCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        WifiCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for WifiCommandJSON", &generator);
    ...
    static WifiCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        WifiCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<WifiCommandJSON>, WifiCommandJSON *, bool> generator("Type WifiCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "WifiCommand") == 0))
                throw("The key field has a value other than `WifiCommand'.");
            WifiCommandJSON *result = new WifiCommandJSON();
            assert(result != NULL);
            RCHandle<WifiCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraWifiCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(WifiCommandJSON *result)
          {
        ...
        virtual void handle_result(WifiCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the WifiCommand class"), unknownFieldGenerator(ignore_extras)
          {
