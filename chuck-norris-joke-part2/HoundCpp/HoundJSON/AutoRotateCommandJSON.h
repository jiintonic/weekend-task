/* file "AutoRotateCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef AUTOROTATECOMMANDJSON_H
#define AUTOROTATECOMMANDJSON_H
...
class AutoRotateCommandJSON : public DeviceControlCommandJSON
  {
  public:
  ...
  private:
    ...
    AutoRotateCommandJSON(const AutoRotateCommandJSON &);
    AutoRotateCommandJSON & operator=(const AutoRotateCommandJSON &other);
  ...
  public:
    AutoRotateCommandJSON(void);
    virtual ~AutoRotateCommandJSON(void);
    ...
    virtual size_t extraAutoRotateCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraAutoRotateCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraAutoRotateCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraAutoRotateCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraAutoRotateCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraAutoRotateCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraAutoRotateCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraAutoRotateCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static AutoRotateCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static AutoRotateCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        AutoRotateCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for AutoRotateCommandJSON", &generator);
    ...
    static AutoRotateCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        AutoRotateCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<AutoRotateCommandJSON>, AutoRotateCommandJSON *, bool> generator("Type AutoRotateCommand", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getDeviceControlCommandJSONKey().c_str(), "AutoRotateCommand") == 0))
                throw("The key field has a value other than `AutoRotateCommand'.");
            AutoRotateCommandJSON *result = new AutoRotateCommandJSON();
            assert(result != NULL);
            RCHandle<AutoRotateCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraAutoRotateCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(AutoRotateCommandJSON *result)
          {
        ...
        virtual void handle_result(AutoRotateCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : DeviceControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the AutoRotateCommand class"), unknownFieldGenerator(ignore_extras)
          {
