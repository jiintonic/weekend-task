/* file "UberCancelCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UBERCANCELCOMMANDJSON_H
#define UBERCANCELCOMMANDJSON_H
...
class UberCancelCommandJSON : public UberCommandJSON
  {
  public:
    class TypeNativeDataJSON : public ReferenceCounted
      ...
      public:
        ...
        class FieldGeneratorStatus : public JSONStringGenerator
          ...
          public:
            Generator(bool ignore_extras = false) : fieldGeneratorStatus("field \"Status\" of the TypeNativeData class"), unknownFieldGenerator(ignore_extras)
              {
  ...
  private:
    ...
    UberCancelCommandJSON(const UberCancelCommandJSON &);
    UberCancelCommandJSON & operator=(const UberCancelCommandJSON &other);
  ...
  public:
    UberCancelCommandJSON(void);
    virtual ~UberCancelCommandJSON(void);
    ...
    virtual size_t extraUberCancelCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUberCancelCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUberCancelCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUberCancelCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUberCancelCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraUberCancelCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraUberCancelCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraUberCancelCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UberCancelCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UberCancelCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UberCancelCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for UberCancelCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static UberCancelCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UberCancelCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UberCancelCommandJSON>, UberCancelCommandJSON *, bool> generator("Type UberCancelCommand", ignore_extras);
    ...
    class Generator : public UberCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberCommandJSONKey().c_str(), "UberCancel") == 0))
                throw("The key field has a value other than `UberCancel'.");
            UberCancelCommandJSON *result = new UberCancelCommandJSON();
            assert(result != NULL);
            RCHandle<UberCancelCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUberCancelCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(UberCancelCommandJSON *result)
          {
        ...
        virtual void handle_result(UberCancelCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the UberCancelCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
