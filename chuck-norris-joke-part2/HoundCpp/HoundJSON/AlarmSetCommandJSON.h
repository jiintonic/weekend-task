/* file "AlarmSetCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef ALARMSETCOMMANDJSON_H
#define ALARMSETCOMMANDJSON_H
...
class AlarmSetCommandJSON : public AlarmCommandJSON
  {
  public:
        ...
        void initAlarms(void)
          {
            if (flagHasAlarms)
              {
                for (size_t num2 = 0; num2 < storeAlarms.size(); ++num2)
                  {
                    storeAlarms[num2]->remove_reference();
                  }
              }
            flagHasAlarms = true;
            storeAlarms.clear();
        ...
        class Generator : public JSONObjectGenerator
          ...
          protected:
            ...
            virtual JSONHandler *start_known_field(const char *field_name)
              {
                if (strcmp(field_name, "Alarms") == 0)
                    return &fieldGeneratorAlarms;
                return NULL;
              }

          public:
            Generator(bool ignore_extras = false) : fieldGeneratorAlarms("field \"Alarms\" of the TypeNativeData class", ignore_extras), unknownFieldGenerator(ignore_extras)
              {
  ...
  private:
    ...
    AlarmSetCommandJSON(const AlarmSetCommandJSON &);
    AlarmSetCommandJSON & operator=(const AlarmSetCommandJSON &other);
  ...
  public:
    AlarmSetCommandJSON(void);
    virtual ~AlarmSetCommandJSON(void);
    ...
    virtual size_t extraAlarmSetCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraAlarmSetCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraAlarmSetCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraAlarmSetCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraAlarmSetCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraAlarmSetCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraAlarmSetCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraAlarmSetCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static AlarmSetCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static AlarmSetCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        AlarmSetCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for AlarmSetCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static AlarmSetCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        AlarmSetCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<AlarmSetCommandJSON>, AlarmSetCommandJSON *, bool> generator("Type AlarmSetCommand", ignore_extras);
    ...
    class Generator : public AlarmCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getAlarmCommandJSONKey().c_str(), "AlarmSetCommand") == 0))
                throw("The key field has a value other than `AlarmSetCommand'.");
            AlarmSetCommandJSON *result = new AlarmSetCommandJSON();
            assert(result != NULL);
            RCHandle<AlarmSetCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraAlarmSetCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(AlarmSetCommandJSON *result)
          {
        ...
        virtual void handle_result(AlarmSetCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : AlarmCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the AlarmSetCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
