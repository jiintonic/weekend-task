/* file "RecipientInfoNotUnderstoodCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef RECIPIENTINFONOTUNDERSTOODCOMMANDJSON_H
#define RECIPIENTINFONOTUNDERSTOODCOMMANDJSON_H
...
class RecipientInfoNotUnderstoodCommandJSON : public CommunicationCommandJSON
  {
  public:
  ...
  private:
    ...
    RecipientInfoNotUnderstoodCommandJSON(const RecipientInfoNotUnderstoodCommandJSON &);
    RecipientInfoNotUnderstoodCommandJSON & operator=(const RecipientInfoNotUnderstoodCommandJSON &other);
  ...
  public:
    RecipientInfoNotUnderstoodCommandJSON(void);
    virtual ~RecipientInfoNotUnderstoodCommandJSON(void);
    ...
    virtual size_t extraRecipientInfoNotUnderstoodCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraRecipientInfoNotUnderstoodCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraRecipientInfoNotUnderstoodCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraRecipientInfoNotUnderstoodCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraRecipientInfoNotUnderstoodCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraRecipientInfoNotUnderstoodCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraRecipientInfoNotUnderstoodCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraRecipientInfoNotUnderstoodCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static RecipientInfoNotUnderstoodCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static RecipientInfoNotUnderstoodCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        RecipientInfoNotUnderstoodCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for RecipientInfoNotUnderstoodCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static RecipientInfoNotUnderstoodCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        RecipientInfoNotUnderstoodCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<RecipientInfoNotUnderstoodCommandJSON>, RecipientInfoNotUnderstoodCommandJSON *, bool> generator("Type RecipientInfoNotUnderstoodCommand", ignore_extras);
    ...
    class Generator : public CommunicationCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getCommunicationCommandJSONKey().c_str(), "RecipientInfoNotUnderstoodCommand") == 0))
                throw("The key field has a value other than `RecipientInfoNotUnderstoodCommand'.");
            RecipientInfoNotUnderstoodCommandJSON *result = new RecipientInfoNotUnderstoodCommandJSON();
            assert(result != NULL);
            RCHandle<RecipientInfoNotUnderstoodCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraRecipientInfoNotUnderstoodCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(RecipientInfoNotUnderstoodCommandJSON *result)
          {
        ...
        virtual void handle_result(RecipientInfoNotUnderstoodCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : CommunicationCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the RecipientInfoNotUnderstoodCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* RECIPIENTINFONOTUNDERSTOODCOMMANDJSON_H */
