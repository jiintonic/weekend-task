/* file "UnitsValue_PressureJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UNITSVALUE_PRESSUREJSON_H
#define UNITSVALUE_PRESSUREJSON_H
...
class UnitsValue_PressureJSON : public ReferenceCounted
  {
  private:
    bool flagHasFormat;
    ...
    UnitsValue_PressureJSON(const UnitsValue_PressureJSON &);
    UnitsValue_PressureJSON & operator=(const UnitsValue_PressureJSON &other);
  ...
  public:
    UnitsValue_PressureJSON(void);
    virtual ~UnitsValue_PressureJSON(void);
    ...
    virtual size_t extraUnitsValue_PressureComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUnitsValue_PressureComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUnitsValue_PressureComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUnitsValue_PressureComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUnitsValue_PressureLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraUnitsValue_PressureLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    void setValue(double new_value)
      {
        flagHasValue = true;
        if (new_value < 0)
            throw("The value for field Value of UnitsValue_PressureJSON is less than the lower bound (0) for that field.");
    ...
    void setValueText(std::string new_value)
      {
        ...
        if (!(JSONWriter::is_valid_number_format(new_value.c_str())))
            throw("The text value for field Value of UnitsValue_PressureJSON is not a valid number.");
    ...
    virtual void extraUnitsValue_PressureAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraUnitsValue_PressureSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UnitsValue_PressureJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UnitsValue_PressureJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UnitsValue_PressureJSON *result;
          {
            ...
            parse_json_value(text, "Text for UnitsValue_PressureJSON", &generator);
    ...
    static UnitsValue_PressureJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UnitsValue_PressureJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UnitsValue_PressureJSON>, UnitsValue_PressureJSON *, bool> generator("Type UnitsValue_Pressure", ignore_extras);
    ...
    class Generator : public JSONObjectGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            ...
            assert(result != NULL);
            RCHandle<UnitsValue_PressureJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUnitsValue_PressureAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(UnitsValue_PressureJSON *result)
          {
        ...
        virtual void handle_result(UnitsValue_PressureJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : fieldGeneratorFormat("field \"Format\" of the UnitsValue_Pressure class"), fieldGeneratorUnits("field \"Units\" of the UnitsValue_Pressure class"), fieldGeneratorWrittenUnits("field \"WrittenUnits\" of the UnitsValue_Pressure class"), fieldGeneratorSpokenUnits("field \"SpokenUnits\" of the UnitsValue_Pressure class"), fieldGeneratorValue("field \"Value\" of the UnitsValue_Pressure class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* UNITSVALUE_PRESSUREJSON_H */
