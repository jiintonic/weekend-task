/* file "HangmanNotUnderstoodCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef HANGMANNOTUNDERSTOODCOMMANDJSON_H
#define HANGMANNOTUNDERSTOODCOMMANDJSON_H
...
class HangmanNotUnderstoodCommandJSON : public HangmanCommandJSON
  {
  public:
  ...
  private:
    ...
    HangmanNotUnderstoodCommandJSON(const HangmanNotUnderstoodCommandJSON &);
    HangmanNotUnderstoodCommandJSON & operator=(const HangmanNotUnderstoodCommandJSON &other);
  ...
  public:
    HangmanNotUnderstoodCommandJSON(void);
    virtual ~HangmanNotUnderstoodCommandJSON(void);
    ...
    virtual size_t extraHangmanNotUnderstoodCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraHangmanNotUnderstoodCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraHangmanNotUnderstoodCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraHangmanNotUnderstoodCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraHangmanNotUnderstoodCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraHangmanNotUnderstoodCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraHangmanNotUnderstoodCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraHangmanNotUnderstoodCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static HangmanNotUnderstoodCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static HangmanNotUnderstoodCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        HangmanNotUnderstoodCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for HangmanNotUnderstoodCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static HangmanNotUnderstoodCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        HangmanNotUnderstoodCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<HangmanNotUnderstoodCommandJSON>, HangmanNotUnderstoodCommandJSON *, bool> generator("Type HangmanNotUnderstoodCommand", ignore_extras);
    ...
    class Generator : public HangmanCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getHangmanCommandJSONKey().c_str(), "HangmanNotUnderstoodGameCommand") == 0))
                throw("The key field has a value other than `HangmanNotUnderstoodGameCommand'.");
            HangmanNotUnderstoodCommandJSON *result = new HangmanNotUnderstoodCommandJSON();
            assert(result != NULL);
            RCHandle<HangmanNotUnderstoodCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraHangmanNotUnderstoodCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(HangmanNotUnderstoodCommandJSON *result)
          {
        ...
        virtual void handle_result(HangmanNotUnderstoodCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : HangmanCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the HangmanNotUnderstoodCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* HANGMANNOTUNDERSTOODCOMMANDJSON_H */
