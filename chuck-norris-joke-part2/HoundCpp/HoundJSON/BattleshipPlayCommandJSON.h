/* file "BattleshipPlayCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BATTLESHIPPLAYCOMMANDJSON_H
#define BATTLESHIPPLAYCOMMANDJSON_H
...
class BattleshipPlayCommandJSON : public BattleshipCommandJSON
  {
  private:
    ...
    BattleshipPlayCommandJSON(const BattleshipPlayCommandJSON &);
    BattleshipPlayCommandJSON & operator=(const BattleshipPlayCommandJSON &other);
  ...
  public:
    BattleshipPlayCommandJSON(void);
    virtual ~BattleshipPlayCommandJSON(void);
    ...
    virtual size_t extraBattleshipPlayCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBattleshipPlayCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBattleshipPlayCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBattleshipPlayCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBattleshipPlayCommandLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraBattleshipPlayCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBattleshipPlayCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraBattleshipPlayCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BattleshipPlayCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BattleshipPlayCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BattleshipPlayCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BattleshipPlayCommandJSON", &generator);
    ...
    static BattleshipPlayCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BattleshipPlayCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BattleshipPlayCommandJSON>, BattleshipPlayCommandJSON *, bool> generator("Type BattleshipPlayCommand", ignore_extras);
    ...
    class Generator : public BattleshipCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBattleshipCommandJSONKey().c_str(), "BattleshipPlayCommand") == 0))
                throw("The key field has a value other than `BattleshipPlayCommand'.");
            BattleshipPlayCommandJSON *result = new BattleshipPlayCommandJSON();
            assert(result != NULL);
            RCHandle<BattleshipPlayCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBattleshipPlayCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(BattleshipPlayCommandJSON *result)
          {
        ...
        virtual void handle_result(BattleshipPlayCommandJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : BattleshipCommandJSON::Generator(ignore_extras), fieldGeneratorUserData("field \"UserData\" of the BattleshipPlayCommand class", ignore_extras), fieldGeneratorComputerData("field \"ComputerData\" of the BattleshipPlayCommand class", ignore_extras), fieldGeneratorStartDateAndOrTime("field \"StartDateAndOrTime\" of the BattleshipPlayCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BATTLESHIPPLAYCOMMANDJSON_H */
