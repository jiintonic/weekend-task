/* file "CarControlClimateModeNotUnderstoodJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef CARCONTROLCLIMATEMODENOTUNDERSTOODJSON_H
#define CARCONTROLCLIMATEMODENOTUNDERSTOODJSON_H
...
class CarControlClimateModeNotUnderstoodJSON : public CarControlCommandJSON
  {
  public:
  ...
  private:
    ...
    CarControlClimateModeNotUnderstoodJSON(const CarControlClimateModeNotUnderstoodJSON &);
    CarControlClimateModeNotUnderstoodJSON & operator=(const CarControlClimateModeNotUnderstoodJSON &other);
  ...
  public:
    CarControlClimateModeNotUnderstoodJSON(void);
    virtual ~CarControlClimateModeNotUnderstoodJSON(void);
    ...
    virtual size_t extraCarControlClimateModeNotUnderstoodComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraCarControlClimateModeNotUnderstoodComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraCarControlClimateModeNotUnderstoodComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraCarControlClimateModeNotUnderstoodComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraCarControlClimateModeNotUnderstoodLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraCarControlClimateModeNotUnderstoodLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraCarControlClimateModeNotUnderstoodAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraCarControlClimateModeNotUnderstoodSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static CarControlClimateModeNotUnderstoodJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static CarControlClimateModeNotUnderstoodJSON *from_text(const char *text, bool ignore_extras = false)
      {
        CarControlClimateModeNotUnderstoodJSON *result;
          {
            ...
            parse_json_value(text, "Text for CarControlClimateModeNotUnderstoodJSON", &generator);
    ...
    static CarControlClimateModeNotUnderstoodJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        CarControlClimateModeNotUnderstoodJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<CarControlClimateModeNotUnderstoodJSON>, CarControlClimateModeNotUnderstoodJSON *, bool> generator("Type CarControlClimateModeNotUnderstood", ignore_extras);
    ...
    class FieldGeneratorCommandType : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getCarControlCommandJSONKey().c_str(), "CarControlClimateModeNotUnderstood") == 0))
                throw("The key field has a value other than `CarControlClimateModeNotUnderstood'.");
            CarControlClimateModeNotUnderstoodJSON *result = new CarControlClimateModeNotUnderstoodJSON();
            assert(result != NULL);
            RCHandle<CarControlClimateModeNotUnderstoodJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraCarControlClimateModeNotUnderstoodAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(CarControlClimateModeNotUnderstoodJSON *result)
          {
        ...
        virtual void handle_result(CarControlClimateModeNotUnderstoodJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : CarControlCommandJSON::Generator(ignore_extras), fieldGeneratorCommandType("field \"CommandType\" of the CarControlClimateModeNotUnderstood class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* CARCONTROLCLIMATEMODENOTUNDERSTOODJSON_H */
