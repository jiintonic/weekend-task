/* file "HomeAutomationMotionStatusJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef HOMEAUTOMATIONMOTIONSTATUSJSON_H
#define HOMEAUTOMATIONMOTIONSTATUSJSON_H
...
class HomeAutomationMotionStatusJSON : public ReferenceCounted
  {
  public:
  ...
  private:
    ...
    HomeAutomationMotionStatusJSON(const HomeAutomationMotionStatusJSON &);
    HomeAutomationMotionStatusJSON & operator=(const HomeAutomationMotionStatusJSON &other);

    void  fromJSONRecentStatus(JSONValue *json_value, bool ignore_extras = false);


  public:
    HomeAutomationMotionStatusJSON(void);
    virtual ~HomeAutomationMotionStatusJSON(void);
    ...
    virtual size_t extraHomeAutomationMotionStatusComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraHomeAutomationMotionStatusComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraHomeAutomationMotionStatusComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraHomeAutomationMotionStatusComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraHomeAutomationMotionStatusLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraHomeAutomationMotionStatusLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraHomeAutomationMotionStatusAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraHomeAutomationMotionStatusSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static HomeAutomationMotionStatusJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static HomeAutomationMotionStatusJSON *from_text(const char *text, bool ignore_extras = false)
      {
        HomeAutomationMotionStatusJSON *result;
          {
            ...
            parse_json_value(text, "Text for HomeAutomationMotionStatusJSON", &generator);
    ...
    static HomeAutomationMotionStatusJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        HomeAutomationMotionStatusJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<HomeAutomationMotionStatusJSON>, HomeAutomationMotionStatusJSON *, bool> generator("Type HomeAutomationMotionStatus", ignore_extras);
    ...
    class FieldGeneratorRecentStatus : public JSONStringGenerator
      ...
      protected:
        ...
        void finish(void)
          {
            ...
            assert(result != NULL);
            RCHandle<HomeAutomationMotionStatusJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraHomeAutomationMotionStatusAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(HomeAutomationMotionStatusJSON *result)
          {
        ...
        virtual void handle_result(HomeAutomationMotionStatusJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : fieldGeneratorRecentStatus("field \"RecentStatus\" of the HomeAutomationMotionStatus class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* HOMEAUTOMATIONMOTIONSTATUSJSON_H */
