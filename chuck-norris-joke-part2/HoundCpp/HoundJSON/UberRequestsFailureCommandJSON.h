/* file "UberRequestsFailureCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef UBERREQUESTSFAILURECOMMANDJSON_H
#define UBERREQUESTSFAILURECOMMANDJSON_H
...
class UberRequestsFailureCommandJSON : public UberRequestsCommandJSON
  {
  public:
  ...
  private:
    ...
    UberRequestsFailureCommandJSON(const UberRequestsFailureCommandJSON &);
    UberRequestsFailureCommandJSON & operator=(const UberRequestsFailureCommandJSON &other);
  ...
  public:
    UberRequestsFailureCommandJSON(void);
    virtual ~UberRequestsFailureCommandJSON(void);
    ...
    virtual size_t extraUberRequestsFailureCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraUberRequestsFailureCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraUberRequestsFailureCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraUberRequestsFailureCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraUberRequestsFailureCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraUberRequestsFailureCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraUberRequestsFailureCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraUberRequestsFailureCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static UberRequestsFailureCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static UberRequestsFailureCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        UberRequestsFailureCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for UberRequestsFailureCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static UberRequestsFailureCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        UberRequestsFailureCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<UberRequestsFailureCommandJSON>, UberRequestsFailureCommandJSON *, bool> generator("Type UberRequestsFailureCommand", ignore_extras);
    ...
    class Generator : public UberRequestsCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getUberRequestsCommandJSONKey().c_str(), "UberRequestsFailure") == 0))
                throw("The key field has a value other than `UberRequestsFailure'.");
            UberRequestsFailureCommandJSON *result = new UberRequestsFailureCommandJSON();
            assert(result != NULL);
            RCHandle<UberRequestsFailureCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraUberRequestsFailureCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(UberRequestsFailureCommandJSON *result)
          {
        ...
        virtual void handle_result(UberRequestsFailureCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : UberRequestsCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the UberRequestsFailureCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* UBERREQUESTSFAILURECOMMANDJSON_H */
