/* file "GamesMenuShowMenuCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef GAMESMENUSHOWMENUCOMMANDJSON_H
#define GAMESMENUSHOWMENUCOMMANDJSON_H
...
class GamesMenuShowMenuCommandJSON : public GamesMenuCommandJSON
  {
  public:
  ...
  private:
    ...
    GamesMenuShowMenuCommandJSON(const GamesMenuShowMenuCommandJSON &);
    GamesMenuShowMenuCommandJSON & operator=(const GamesMenuShowMenuCommandJSON &other);
  ...
  public:
    GamesMenuShowMenuCommandJSON(void);
    virtual ~GamesMenuShowMenuCommandJSON(void);
    ...
    virtual size_t extraGamesMenuShowMenuCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraGamesMenuShowMenuCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraGamesMenuShowMenuCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraGamesMenuShowMenuCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraGamesMenuShowMenuCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraGamesMenuShowMenuCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraGamesMenuShowMenuCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraGamesMenuShowMenuCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static GamesMenuShowMenuCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static GamesMenuShowMenuCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        GamesMenuShowMenuCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for GamesMenuShowMenuCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static GamesMenuShowMenuCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        GamesMenuShowMenuCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<GamesMenuShowMenuCommandJSON>, GamesMenuShowMenuCommandJSON *, bool> generator("Type GamesMenuShowMenuCommand", ignore_extras);
    ...
    class Generator : public GamesMenuCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getGamesMenuCommandJSONKey().c_str(), "GamesMenuShowMenuCommand") == 0))
                throw("The key field has a value other than `GamesMenuShowMenuCommand'.");
            GamesMenuShowMenuCommandJSON *result = new GamesMenuShowMenuCommandJSON();
            assert(result != NULL);
            RCHandle<GamesMenuShowMenuCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraGamesMenuShowMenuCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(GamesMenuShowMenuCommandJSON *result)
          {
        ...
        virtual void handle_result(GamesMenuShowMenuCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : GamesMenuCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the GamesMenuShowMenuCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* GAMESMENUSHOWMENUCOMMANDJSON_H */
