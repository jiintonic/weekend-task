/* file "BlackjackStandInformationNuggetJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKSTANDINFORMATIONNUGGETJSON_H
#define BLACKJACKSTANDINFORMATIONNUGGETJSON_H
...
class BlackjackStandInformationNuggetJSON : public BlackjackInformationNuggetJSON
  {
  private:
    ...
    BlackjackStandInformationNuggetJSON(const BlackjackStandInformationNuggetJSON &);
    BlackjackStandInformationNuggetJSON & operator=(const BlackjackStandInformationNuggetJSON &other);
  ...
  public:
    BlackjackStandInformationNuggetJSON(void);
    virtual ~BlackjackStandInformationNuggetJSON(void);
    ...
    virtual size_t extraBlackjackStandInformationNuggetComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackStandInformationNuggetComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackStandInformationNuggetComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackStandInformationNuggetComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackStandInformationNuggetLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraBlackjackStandInformationNuggetLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    void setBankroll(double new_value)
      {
        ...
        if (new_value < 0)
            throw("The value for field Bankroll of BlackjackStandInformationNuggetJSON is less than the lower bound (0) for that field.");
    ...
    void setBankrollText(std::string new_value)
      {
        ...
        if (!(JSONWriter::is_valid_number_format(new_value.c_str())))
            throw("The text value for field Bankroll of BlackjackStandInformationNuggetJSON is not a valid number.");
    ...
    void setPlayerHandTotal(uint8_t new_value)
      {
        ...
        if (new_value < 4)
            throw("The value for field PlayerHandTotal of BlackjackStandInformationNuggetJSON is less than the lower bound (4) for that field.");
        if (new_value > 20)
            throw("The value for field PlayerHandTotal of BlackjackStandInformationNuggetJSON is greater than the upper bound (20) for that field.");
    ...
    void setDealerHandTotal(uint8_t new_value)
      {
        ...
        if (new_value < 17)
            throw("The value for field DealerHandTotal of BlackjackStandInformationNuggetJSON is less than the lower bound (17) for that field.");
        if (new_value > 26)
            throw("The value for field DealerHandTotal of BlackjackStandInformationNuggetJSON is greater than the upper bound (26) for that field.");
    ...
    virtual void extraBlackjackStandInformationNuggetAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraBlackjackStandInformationNuggetSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackStandInformationNuggetJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackStandInformationNuggetJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackStandInformationNuggetJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackStandInformationNuggetJSON", &generator);
    ...
    static BlackjackStandInformationNuggetJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackStandInformationNuggetJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackStandInformationNuggetJSON>, BlackjackStandInformationNuggetJSON *, bool> generator("Type BlackjackStandInformationNugget", ignore_extras);
    ...
    class Generator : public BlackjackInformationNuggetJSON::Generator
      ...
      private:
        ...
        JSONHoldingGenerator<JSONIntegerRangeGenerator<uint8_t, 4, 20>, uint8_t, uint8_t > fieldGeneratorPlayerHandTotal;
        ...
        JSONHoldingGenerator<JSONIntegerRangeGenerator<uint8_t, 17, 26>, uint8_t, uint8_t > fieldGeneratorDealerHandTotal;
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackInformationNuggetJSONKey().c_str(), "BlackjackStandCommand") == 0))
                throw("The key field has a value other than `BlackjackStandCommand'.");
            BlackjackStandInformationNuggetJSON *result = new BlackjackStandInformationNuggetJSON();
            assert(result != NULL);
            RCHandle<BlackjackStandInformationNuggetJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackStandInformationNuggetAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
        ...
        void finish(BlackjackStandInformationNuggetJSON *result)
          {
        ...
        virtual void handle_result(BlackjackStandInformationNuggetJSON *new_result) = 0;
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackInformationNuggetJSON::Generator(ignore_extras), fieldGeneratorImageURL("field \"ImageURL\" of the BlackjackStandInformationNugget class"), fieldGeneratorBankroll("field \"Bankroll\" of the BlackjackStandInformationNugget class"), fieldGeneratorPlayerHand("field \"PlayerHand\" of the BlackjackStandInformationNugget class", ignore_extras), fieldGeneratorPlayerHandTotal("field \"PlayerHandTotal\" of the BlackjackStandInformationNugget class"), fieldGeneratorDealerHand("field \"DealerHand\" of the BlackjackStandInformationNugget class", ignore_extras), fieldGeneratorDealerHandTotal("field \"DealerHandTotal\" of the BlackjackStandInformationNugget class"), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKSTANDINFORMATIONNUGGETJSON_H */
