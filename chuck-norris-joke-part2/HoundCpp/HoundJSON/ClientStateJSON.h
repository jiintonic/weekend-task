...
class ClientStateJSON : public ReferenceCounted
  ...
  private:
    ...
    ClientStateJSON(const ClientStateJSON &);
    ClientStateJSON & operator=(const ClientStateJSON &other);
  ...
  public:
    ...
    bool  hasVerticalState(void) const;
    ClientVerticalStateJSON *  getVerticalState(void);
    const ClientVerticalStateJSON *  getVerticalState(void) const;

    virtual size_t extraClientStateComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraClientStateComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraClientStateComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraClientStateComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraClientStateLookup(const char *field_name)
      {
    ...
    virtual const JSONValue *extraClientStateLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraClientStateAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
    ...
    virtual void extraClientStateSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
        ...
        ClientStateJSON *result;
          {
            ...
            parse_json_value(text, "Text for ClientStateJSON", &generator);
    ...
    class Generator : public JSONObjectGenerator
        ...
        void finish(void)
          {
            ...
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraClientStateAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
      ...
      public:
        Generator(bool ignore_extras = false) : fieldGeneratorVerticalState("field \"VerticalState\" of the ClientState class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
