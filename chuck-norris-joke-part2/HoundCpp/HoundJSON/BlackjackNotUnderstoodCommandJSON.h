/* file "BlackjackNotUnderstoodCommandJSON.h" */

/* Generated automatically by Classy JSON. */


#ifndef BLACKJACKNOTUNDERSTOODCOMMANDJSON_H
#define BLACKJACKNOTUNDERSTOODCOMMANDJSON_H
...
class BlackjackNotUnderstoodCommandJSON : public BlackjackCommandJSON
  {
  public:
  ...
  private:
    ...
    BlackjackNotUnderstoodCommandJSON(const BlackjackNotUnderstoodCommandJSON &);
    BlackjackNotUnderstoodCommandJSON & operator=(const BlackjackNotUnderstoodCommandJSON &other);
  ...
  public:
    BlackjackNotUnderstoodCommandJSON(void);
    virtual ~BlackjackNotUnderstoodCommandJSON(void);
    ...
    virtual size_t extraBlackjackNotUnderstoodCommandComponentCount(void) const
      {
        assert(extraKeys.size() == extraValues.size());
        return extraKeys.size();
      }
    virtual const char *extraBlackjackNotUnderstoodCommandComponentKey(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraKeys[component_num].c_str();
      }
    virtual JSONValue *extraBlackjackNotUnderstoodCommandComponentValue(size_t component_num)
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual const JSONValue *extraBlackjackNotUnderstoodCommandComponentValue(size_t component_num) const
      {
        assert(extraKeys.size() == extraValues.size());
        assert(component_num < extraValues.size());
        return extraValues[component_num];
      }
    virtual JSONValue *extraBlackjackNotUnderstoodCommandLookup(const char *field_name)
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual const JSONValue *extraBlackjackNotUnderstoodCommandLookup(const char *field_name) const
      {
        JSONValue *result = (JSONValue *)(lookup_in_string_index(extraIndex, field_name));
        if (result != NULL)
            result->add_reference();
    ...
    virtual void extraBlackjackNotUnderstoodCommandAppendPair(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    virtual void extraBlackjackNotUnderstoodCommandSetField(const char *key, JSONValue *new_component)
      {
        assert(key != NULL);
        assert(new_component != NULL);
    ...
    static BlackjackNotUnderstoodCommandJSON *from_json(JSONValue *json_value, bool ignore_extras = false);
    static BlackjackNotUnderstoodCommandJSON *from_text(const char *text, bool ignore_extras = false)
      {
        BlackjackNotUnderstoodCommandJSON *result;
          {
            ...
            parse_json_value(text, "Text for BlackjackNotUnderstoodCommandJSON", &generator);
            assert(generator.have_value);
            result = generator.value.referenced();
            result->add_reference();
          };
    ...
    static BlackjackNotUnderstoodCommandJSON *from_file(FILE *fp, const char *file_name, bool ignore_extras = false)
      {
        BlackjackNotUnderstoodCommandJSON *result;
          {
            JSONHoldingGenerator<Generator, RCHandle<BlackjackNotUnderstoodCommandJSON>, BlackjackNotUnderstoodCommandJSON *, bool> generator("Type BlackjackNotUnderstoodCommand", ignore_extras);
    ...
    class Generator : public BlackjackCommandJSON::Generator
      ...
      protected:
        ...
        void finish(void)
          {
            if (!(strcmp(getBlackjackCommandJSONKey().c_str(), "BlackjackNotUnderstoodCommand") == 0))
                throw("The key field has a value other than `BlackjackNotUnderstoodCommand'.");
            BlackjackNotUnderstoodCommandJSON *result = new BlackjackNotUnderstoodCommandJSON();
            assert(result != NULL);
            RCHandle<BlackjackNotUnderstoodCommandJSON> result_holder = result;
            finish(result);
            size_t extra_count = unknownFieldGenerator.field_names.size();
            assert(extra_count == unknownFieldGenerator.field_values.size());
            for (size_t extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result->extraBlackjackNotUnderstoodCommandAppendPair(unknownFieldGenerator.field_names[extra_num].c_str(), unknownFieldGenerator.field_values[extra_num].referenced());
              }
            unknownFieldGenerator.field_names.clear();
            unknownFieldGenerator.field_values.clear();
            destroy_string_index(unknownFieldGenerator.index);
        ...
        void finish(BlackjackNotUnderstoodCommandJSON *result)
          {
        ...
        virtual void handle_result(BlackjackNotUnderstoodCommandJSON *new_result) = 0;
        virtual JSONHandler *start_known_field(const char *field_name)
          {
            if (strcmp(field_name, "NativeData") == 0)
      ...
      public:
        Generator(bool ignore_extras = false) : BlackjackCommandJSON::Generator(ignore_extras), fieldGeneratorNativeData("field \"NativeData\" of the BlackjackNotUnderstoodCommand class", ignore_extras), unknownFieldGenerator(ignore_extras)
          {
...
#endif /* BLACKJACKNOTUNDERSTOODCOMMANDJSON_H */
